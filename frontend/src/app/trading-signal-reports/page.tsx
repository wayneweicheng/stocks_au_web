  ...
  const loadReports = useCallback(async (append = false, reportId?: string) => {
    ...
    try {
      ...
      if (!append && reportId && !items.some((item) => item.public_report_id === reportId)) {
        const requestedQuery = buildTradingSignalReportsQuery({ public_report_id: reportId, current_only: "0", exclude_no_signal: hideNoSignal ? "1" : undefined, limit: "1" });
