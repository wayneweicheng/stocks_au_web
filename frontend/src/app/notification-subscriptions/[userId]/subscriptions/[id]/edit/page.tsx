...
export default function EditSubscriptionPage({ params }: { params: Promise<{ userId: string; id: string }> }) {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const detailRes = await authenticatedFetch(`${baseUrl}/api/subscriptions/${id}`);
        if (!detailRes.ok) throw new Error(`HTTP ${detailRes.status}`);
        const detail = await detailRes.json();
        setEventType(detail.EventType);
        setEntityCode(detail.EntityCode);
        setTriggerOperator(detail.TriggerOperator || "above");
        setTriggerValue(detail.TriggerValue != null ? String(detail.TriggerValue) : "");
        setTriggerValue2(detail.TriggerValue2 != null ? String(detail.TriggerValue2) : "");
        setIncludeKeywords(detail.IncludeKeywords || "");
        setExcludeKeywords(detail.ExcludeKeywords || "");
        setPriority(detail.Priority ?? 0);
        setChannel(detail.NotificationChannel || "");
        setConfigJson(detail.ConfigurationJSON || "");
        setIsActive(Boolean(detail.IsActive));

        // load types for the event type
        const typesRes = await authenticatedFetch(`${baseUrl}/api/subscription-types?eventType=${encodeURIComponent(detail.EventType)}`);
        if (!typesRes.ok) throw new Error(`HTTP ${typesRes.status}`);
        const typesJson = await typesRes.json();
        setTypes(typesJson);
        setSelectedTypeId(detail.SubscriptionTypeID);
      } catch (e: any) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await authenticatedFetch(`${baseUrl}/api/subscriptions/${id}`, {
        method: "PUT",
      ...
      window.location.href = `/notification-subscriptions/${userId}/subscriptions`;
    } catch (e: any) {
        ...
        <div className="mx-auto max-w-3xl px-6 py-10">
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-3">
              <Link href="/notification-subscriptions" className="text-blue-600 hover:underline">Users</Link>
              <span>/</span>
              <Link href={`/notification-subscriptions/${userId}/subscriptions`} className="text-blue-600 hover:underline">Subscriptions</Link>
              <span>/</span>
              <span>Edit</span>
            </div>
            <Link href={`/notification-subscriptions/${userId}/subscriptions`} className="text-blue-600 hover:underline">Back</Link>
          </div>
          {error && (
          ...
          <form onSubmit={handleSubmit} className="grid gap-4">
            <div>
              ...
              <input value={eventType} readOnly className="w-full rounded-md border border-slate-200 bg-slate-50 px-3 py-2 text-sm" />
            </div>
            <div>
              ...
              {selectedType?.Description && <p className="text-xs text-slate-500 mt-1">{selectedType.Description}</p>}
            </div>
            ...
            <div className="flex gap-2">
              <Link href={`/notification-subscriptions/${userId}/subscriptions`} className="rounded-md bg-gray-200 text-gray-800 px-4 py-2 text-sm">Cancel</Link>
