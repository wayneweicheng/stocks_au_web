...
function fmtSignedPct(v: any) { const n = num(v); return n === null ? "-" : `${n >= 0 ? "+" : ""}${n.toFixed(2)}%`; }
...
export default function OptionGexDeltaCapitalTypePage() {
  ...
  const [data, setData] = useState<{ capital_types: string[]; rows: Row[]; table_rows: Row[]; count: number; stock_code: string; qqq_price_change_pct: number | null } | null>(null);
  ...
  return (
    <div className="space-y-6">
      ...
      <Card><CardHeader><CardTitle>GEX Insight Table</CardTitle></CardHeader><CardContent><div className="overflow-x-auto rounded-md border border-slate-200"><table className="min-w-full text-sm"><thead className="bg-slate-50 text-left text-xs uppercase tracking-wide text-slate-500"><tr><th className="px-3 py-3">Date</th><th className="px-3 py-3">Insight</th><th className="px-3 py-3 text-right">BC %</th><th className="px-3 py-3 text-right">BC % Pre</th><th className="px-3 py-3 text-right">SP %</th><th className="px-3 py-3 text-right">QQQ Price Change</th><th className="px-3 py-3 text-right">Close</th></tr></thead><tbody className="divide-y divide-slate-100">{recentTable.map((r) => <tr key={r.ObservationDate}><td className="px-3 py-3">{dateLabel(r.ObservationDate)}</td><td className="px-3 py-3">{r.GEXInsight || "-"}</td><td className="px-3 py-3 text-right">{fmt(r.BC_GEXDeltaPerc)}</td><td className="px-3 py-3 text-right">{fmt(r.BC_GEXDeltaPerc_Pre)}</td><td className="px-3 py-3 text-right">{fmt(r.SP_GEXDeltaPerc)}</td><td className="px-3 py-3 text-right">{fmtSignedPct(data?.qqq_price_change_pct)}</td><td className="px-3 py-3 text-right">{fmt(r.Close)}</td></tr>)}{!recentTable.length ? <tr><td colSpan={7} className="px-3 py-8 text-center text-slate-500">{loading ? "Loading..." : "No data found."}</td></tr> : null}</tbody></table></div></CardContent></Card>
    </div>
  );
}
