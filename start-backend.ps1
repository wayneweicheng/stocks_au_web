# Backend Supervisor - FastAPI/Uvicorn (single service)
...
$StartupLog = Join-Path $AbsoluteLogPath "backend-$(Get-Date -Format 'yyyyMMdd-HHmmss').log"
...
while ($true) {
    ...
    try {
        ...
        # extra cleanup if listener stayed
        Stop-ProcessOnPort -Port $Port
    }
    ...
    $restartCount++
    Write-Log "Restarting backend (attempt $restartCount) after $RestartCooldown seconds..."
