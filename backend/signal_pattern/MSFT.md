...
This prompt was generated for `MSFT.US` with requested as-of date `2026-06-02` from a fresh SQL Server extraction for this ticker. Candidate rules and the feature-score gate were evaluated separately on `TomorrowChange`, `Next2DaysChange`, and `Next5DaysChange`. The selected primary target for this ticker is `TomorrowChange` (next 1 trading day). This section is research metadata only; do not use it as today's market state during live or historical replay analysis.
...
- Daily feature rows: 832 total, 831 selected-target labeled rows from 2023-01-04 to 2026-05-29.
- Train split: 2023-01-04 to 2025-05-21, 581 rows.
- Validation split: 2025-05-22 to 2025-11-21, 125 rows.
- Test split: 2025-11-24 to 2026-05-29, 125 rows.
...
- 1-day: feature-score test win 54.0%, coverage 100/125 (80.0%), selection rank value 548.17, accepted robust patterns 4.
- 5-day: feature-score test win 49.4%, coverage 77/125 (61.6%), selection rank value 499.88, accepted robust patterns 6.
- 2-day: feature-score test win 42.5%, coverage 80/125 (64.0%), selection rank value 431.40, accepted robust patterns 6.
- Baseline selected-target win rate: train 53.9%, validation 52.8%, test 48.8%.
- Baseline tomorrow win rate: train 53.9%, validation 52.8%, test 48.8%.
- Baseline 2-day win rate: train 54.9%, validation 56.5%, test 44.0%.
- Baseline 5-day win rate: train 59.3%, validation 66.1%, test 47.2%.
- Selective feature-score gate: threshold 0.50 on the trained probability score.
- Feature-score train performance on selected target: win 65.1%, coverage 355/581 (61.1%), avg selected return +0.318%.
- Feature-score validation performance on selected target: win 46.7%, coverage 60/125 (48.0%), avg selected return -0.074%.
- Feature-score test performance on selected target: win 54.0%, coverage 100/125 (80.0%), avg selected return +0.170%.
- Large option trade rows sampled: 1000.
- Latest OI-change records sampled: 100. Positive call OI change 166683; positive put OI change 24408; near-term call additions 52025; near-term put additions 11188.
- Top current OI records sampled: 50.
- 30-minute bars sampled: 128.
- Broad feature audit rules tested: 290 single-feature rules and 19 two-feature combinations.

### Accepted Patterns

- `GEX_BigRise = 1` (1-day): train n=65, avg +0.076%, win 55.4%; validation n=58, avg +0.092%, win 55.2%; test n=56, avg +0.253%, win 53.6%; test 1-day avg +0.253%, test 2-day avg +0.132%, test 5-day avg -0.554%.
...
- `BB_PercentB <= train_q20 (0.3127)` (1-day): train n=104, avg +0.308%, win 57.7%; validation n=24, avg +0.032%, win 58.3%; test n=51, avg +0.004%, win 49.0%; test 1-day avg +0.004%, test 2-day avg +0.041%, test 5-day avg -0.292%.
...
The model is intentionally selective. Historical test coverage was 100/125 (80.0%), which means many days did not qualify for a high-confidence forecast. In the live report, you must clearly label whether today's setup is inside or outside the validated high-confidence regime.
...
- `GEX_ZScore_60day`: model weight +0.4240
- `RSI`: model weight +0.4012
- `GEX_Percentile_High`: model weight -0.3115
- `GEX_Percentile`: model weight +0.2940
- `MACD_Line`: model weight -0.2807
- `GEX_Falling`: model weight +0.2738
- `GEX_Rising`: model weight -0.2670
- `GEX_Percentile_VeryLow`: model weight +0.2644
- `Is_Potential_Swing_Up`: model weight +0.2403
- `Prev10DaysChange`: model weight -0.2385
- `GEXChange_Positive`: model weight +0.2186
- `Stock_DarkPoolIndex`: model weight -0.2168

### Rejected Or Downgraded Patterns

- `SMA20_Above_SMA50 = 0 AND BB_PercentB <= train_q20 (0.3127)` (1-day): train n=37, avg +0.783%, win 70.3%; validation n=6, avg -0.068%, win 66.7%; test n=49, avg -0.080%, win 46.9%; test 1-day avg -0.080%, test 2-day avg -0.112%, test 5-day avg -0.432%.
- `BB_PercentB <= train_q20 (0.3127) AND BB_Bandwidth <= train_q20 (0.0628)` (1-day): train n=20, avg +0.626%, win 60.0%; validation n=6, avg -0.128%, win 50.0%; test n=7, avg -0.226%, win 28.6%; test 1-day avg -0.226%, test 2-day avg +0.067%, test 5-day avg -0.746%.
- `GEX_Negative = 1` (1-day): train n=21, avg +0.587%, win 57.1%; validation n=13, avg -0.481%, win 30.8%; test n=19, avg -0.288%, win 52.6%; test 1-day avg -0.288%, test 2-day avg +0.108%, test 5-day avg +0.061%.
- `VIX >= train_q90 (21.98)` (1-day): train n=59, avg +0.577%, win 62.7%; validation n=7, avg -0.576%, win 42.9%; test n=24, avg -0.366%, win 33.3%; test 1-day avg -0.366%, test 2-day avg -0.680%, test 5-day avg -1.438%.
- `BB_PercentB <= train_q10 (0.1624)` (1-day): train n=52, avg +0.539%, win 57.7%; validation n=12, avg -0.193%, win 50.0%; test n=26, avg -0.252%, win 46.2%; test 1-day avg -0.252%, test 2-day avg -0.216%, test 5-day avg -0.566%.
- `Price_Above_SMA50 = 0 AND VIX >= train_q80 (19.8)` (1-day): train n=68, avg +0.393%, win 58.8%; validation n=15, avg +0.170%, win 60.0%; test n=37, avg -0.035%, win 43.2%; test 1-day avg -0.035%, test 2-day avg -0.070%, test 5-day avg -0.257%.
...
- **Historical Evidence:** `GEX_BigRise = 1` (1-day): train n=65, avg +0.076%, win 55.4%; validation n=58, avg +0.092%, win 55.2%; test n=56, avg +0.253%, win 53.6%; test 1-day avg +0.253%, test 2-day avg +0.132%, test 5-day avg -0.554%.
...
#### BB_PercentB <= train_q20 (0.3127)
- **Tier:** Tier 2
- **Signal:** Bullish for the next 1 trading day.
- **Historical Evidence:** `BB_PercentB <= train_q20 (0.3127)` (1-day): train n=104, avg +0.308%, win 57.7%; validation n=24, avg +0.032%, win 58.3%; test n=51, avg +0.004%, win 49.0%; test 1-day avg +0.004%, test 2-day avg +0.041%, test 5-day avg -0.292%.
- **Rationale:** Use this as an explainable stock-specific 1-day market-flow rule for MSFT. Cross-check the other horizons for timing, confirmation, or conflict.
- **Priority:** Medium; downgrade if current option flow strongly contradicts it.
...
- **Historical Test Coverage:** 100/125 (80.0%)
- **Historical Test Win Rate When Covered:** 54.0%
