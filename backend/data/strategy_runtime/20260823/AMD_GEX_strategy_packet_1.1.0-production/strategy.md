# AMD GEX Capital-Type Feature-Aware Strategy — 1.1.0-production
...
Base capital-type triggers are unchanged except for confidence/gating of `AMD_SELL_PUT_SUPPORTED_STRENGTH`.

**HIGH predicate:** `QQQ.GEX_ZScore_60day < 0`

- `AMD_SELL_PUT_SUPPORTED_STRENGTH_HIGH` → PLAN_ENTRY / production notification.
- `AMD_SELL_PUT_SUPPORTED_STRENGTH_STANDARD` → PLAN_ENTRY / production notification.

The base sell-put-supported-strength signal remains actionable. QQQ aggregate GEX below its 60-day z-score mean elevates it to HIGH confidence; otherwise it remains STANDARD.

## Data readiness

At D1 07:30 America/New_York, require the previous XNYS session D0 to be complete in `StockDB_US.Transform.OptionGEXChangeCapitalType` and require D0 feature rows in `StockDB_US.Analysis.GEX_Features` for **AMD** and **QQQ**. Missing/null feature inputs are **NOT_READY**; do not silently downgrade HIGH to STANDARD/WEAK. Source correction creates a new immutable evaluation revision.
