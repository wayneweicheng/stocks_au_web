# MU GEX Capital-Type Feature-Aware Strategy — 1.1.0-production
...
Base capital-type triggers are unchanged except for confidence/gating of `MU_DOWN_DAY_INTENT_REVERSAL`.

**HIGH predicate:** `QQQ.GEX_Trending_Up == 1 OR MU.GEX_ZScore < -1.0`

- `MU_DOWN_DAY_INTENT_REVERSAL_HIGH` → PLAN_ENTRY / production notification.
- `MU_DOWN_DAY_INTENT_REVERSAL_WEAK` → WATCH / production notification.

The original down-day intent-reversal setup is actionable only in the historically robust feature regime. The weak regime had negative historical expectancy and is notification-only WATCH.

## Data readiness

At D1 07:30 America/New_York, require the previous XNYS session D0 to be complete in `StockDB_US.Transform.OptionGEXChangeCapitalType` and require D0 feature rows in `StockDB_US.Analysis.GEX_Features` for **MU** and **QQQ**. Missing/null feature inputs are **NOT_READY**; do not silently downgrade HIGH to STANDARD/WEAK. Source correction creates a new immutable evaluation revision.
