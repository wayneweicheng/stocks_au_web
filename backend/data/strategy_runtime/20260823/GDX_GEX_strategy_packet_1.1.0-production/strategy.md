# GDX GEX Capital-Type Feature-Aware Strategy — 1.1.0-production
...
Base capital-type triggers are unchanged except for confidence/gating of `GDX_CALL_BUY_SHARE_SURGE`.

**HIGH predicate:** `SPY.Price_Above_SMA20 == 0`

- `GDX_CALL_BUY_SHARE_SURGE_HIGH` → PLAN_ENTRY / production notification.
- `GDX_CALL_BUY_SHARE_SURGE_STANDARD` → PLAN_ENTRY / production notification.

The base GDX call-buy-share-surge long remains actionable. SPY below its D0 SMA20 elevates the signal to HIGH confidence; otherwise it remains STANDARD.

## Data readiness

At D1 07:30 America/New_York, require the previous XNYS session D0 to be complete in `StockDB_US.Transform.OptionGEXChangeCapitalType` and require D0 feature rows in `StockDB_US.Analysis.GEX_Features` for **GDX** and **SPY**. Missing/null feature inputs are **NOT_READY**; do not silently downgrade HIGH to STANDARD/WEAK. Source correction creates a new immutable evaluation revision.
