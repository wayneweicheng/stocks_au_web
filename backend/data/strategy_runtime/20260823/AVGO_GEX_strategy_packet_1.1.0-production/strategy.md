# AVGO GEX Capital-Type Feature-Aware Strategy — 1.1.0-production
...
Base capital-type triggers are unchanged except for confidence/gating of `AVGO_BULLISH_INTENT_RALLY_EXHAUSTION`.

**HIGH predicate:** `QQQ.Is_Potential_Swing_Up == 1 OR QQQ.TodayChange < 0`

- `AVGO_BULLISH_INTENT_RALLY_EXHAUSTION_HIGH` → PLAN_ENTRY / production notification.
- `AVGO_BULLISH_INTENT_RALLY_EXHAUSTION_WEAK` → WATCH / production notification.

The bearish rally-exhaustion setup is actionable only when QQQ confirms with causal GEX/price divergence or a down D0 session. The weak regime historically had no durable positive expectancy.

## Data readiness

At D1 07:30 America/New_York, require the previous XNYS session D0 to be complete in `StockDB_US.Transform.OptionGEXChangeCapitalType` and require D0 feature rows in `StockDB_US.Analysis.GEX_Features` for **AVGO** and **QQQ**. Missing/null feature inputs are **NOT_READY**; do not silently downgrade HIGH to STANDARD/WEAK. Source correction creates a new immutable evaluation revision.
