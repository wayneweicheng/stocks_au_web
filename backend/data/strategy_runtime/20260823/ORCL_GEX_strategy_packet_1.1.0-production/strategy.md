# ORCL GEX Capital-Type Feature-Aware Strategy — 1.1.0-production
...
Base capital-type triggers are unchanged except for confidence/gating of `ORCL_FAILED_BULLISH_SUPPORT`.

**HIGH predicate:** `ORCL.GEXChange < -10.0`

- `ORCL_FAILED_BULLISH_SUPPORT_HIGH` → PLAN_ENTRY / production notification.
- `ORCL_FAILED_BULLISH_SUPPORT_STANDARD` → PLAN_ENTRY / production notification.

The base failed-bullish-support short remains actionable. A signed aggregate-GEX change below -10% elevates the signal to HIGH confidence; otherwise it remains STANDARD.

## Data readiness

At D1 07:30 America/New_York, require the previous XNYS session D0 to be complete in `StockDB_US.Transform.OptionGEXChangeCapitalType` and require D0 feature rows in `StockDB_US.Analysis.GEX_Features` for **ORCL** and **QQQ**. Missing/null feature inputs are **NOT_READY**; do not silently downgrade HIGH to STANDARD/WEAK. Source correction creates a new immutable evaluation revision.
