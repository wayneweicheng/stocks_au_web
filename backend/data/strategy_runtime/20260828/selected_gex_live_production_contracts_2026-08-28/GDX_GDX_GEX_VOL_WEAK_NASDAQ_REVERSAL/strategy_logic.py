"""Production notification-only reference logic for GEX_GDX_VOL_NASDAQ_REVERSAL 1.0.0-production.
Broker execution is HARD_DISABLED. User decides manual order placement.
"""
STRATEGY_CODE='GEX_GDX_VOL_NASDAQ_REVERSAL'
VERSION='1.0.0-production'
DEPLOYMENT_TARGET="PRODUCTION"
ENABLED=True
NOTIFICATIONS_ENABLED=True
BROKER_EXECUTION="HARD_DISABLED"
SIGNAL_CODE='GDX_GEX_VOL_WEAK_NASDAQ_REVERSAL'
DIRECTION='LONG'
HOLDING_PERIOD='D5'
...
def evaluate(features):
    missing=[x for x in ['SafeGEXVolRank60', 'NASDAQ_chg5'] if x not in features or features[x] is None]
    if missing:
        return {"state":"NOT_READY","missing":missing}
    matched=bool(features["SafeGEXVolRank60"] >= 0.40 and features["NASDAQ_chg5"] <= -1.0)
    if matched:
