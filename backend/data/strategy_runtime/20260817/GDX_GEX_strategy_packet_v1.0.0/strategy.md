# GDX GEX Capital-Type Strategy Contract

## 1. Strategy identity

- **strategy_code:** `GEX_GDX_CAPITALTYPE`
- **display_name:** GDX GEX Capital-Type Strategy
- **version_code:** `1.0.0`
- **implementation_key:** `gex.gdx.capital_type.v1_0_0`
- **status:** `REVIEW_READY`
- **purpose:** Use prior-session GDX option-GEX capital-type structure to classify next-session directional opportunities; runtime is notification-only.
- **economic/research rationale:** A large causal increase in bought-call share historically precedes multi-day upside in GDX.
- **owner:** Wayne Cheng
...
- **source snapshot identifiers/hashes:** see `source-manifest.json`; combined source hash `e99e485f5feb319e4e1dcd31dcd50e85c192c7850189aef9c9441e6b7881bc50`.
...
|---|---|---|---|---|---|---|---|
| SUBJECT | GDX | NYSE Arca | USD | America/New_York | US market date of source observation | NYSE Arca regular US equity calendar; exchange holidays skipped; America/New_York DST; early close uses last regular-session bar | LONG |
| SOURCE | GDX option GEX capital type | internal/Drive snapshot | USD-derived GEX | America/New_York | ObservationDate = completed US session | Previous US trading session is D0; four capital types BC/BP/SC/SP required | NONE |
| EXECUTION | GDX | NYSE Arca | USD | America/New_York | D1/D2/... are US trading sessions after D0 | Exchange calendar; 07:30 premarket reference; early-close exit uses last regular bar | LONG |
...
|---:|---|---|---|---|---|---|---|
| 1 | `GDX_CALL_BUY_SHARE_SURGE` | `BCShareChange > 0 AND BCShareAbsShockPct60 >= 0.75` | LONG | PLAN_ENTRY | D1 07:30 ET bar Open; signal must be ready before/reference at that time | D5 final regular-session 30m bar Close; no TP/SL | yes |
...
|---|---|---|---|---|---|---|---|
| Close | `GDX-20230815-20260813.csv` / `Close` | Observation-day underlying close | USD | raw; positive price | no rounding | NOT_READY if null | D0; current D0 included |
| VWAP | `GDX-20230815-20260813.csv` / `VWAP` | Observation-day source VWAP | USD | raw; positive price | no rounding | NOT_READY if null | D0; current D0 included |
| BCAbs | `GDX-20230815-20260813.csv` / `GEXDelta where CapitalType=BC` | ABS(GEXDelta) | GEX-delta units | absolute; nonnegative | no rounding | NOT_READY if missing | D0; current D0 included |
| BPAbs | `GDX-20230815-20260813.csv` / `GEXDelta where CapitalType=BP` | ABS(GEXDelta) | GEX-delta units | absolute; nonnegative | no rounding | NOT_READY if missing | D0; current D0 included |
| SCAbs | `GDX-20230815-20260813.csv` / `GEXDelta where CapitalType=SC` | ABS(GEXDelta) | GEX-delta units | absolute; nonnegative | no rounding | NOT_READY if missing | D0; current D0 included |
| SPAbs | `GDX-20230815-20260813.csv` / `GEXDelta where CapitalType=SP` | ABS(GEXDelta) | GEX-delta units | absolute; nonnegative | no rounding | NOT_READY if missing | D0; current D0 included |
...
- **GDX_CALL_BUY_SHARE_SURGE TRUE:** historical ObservationDate `2025-10-31`; relevant inputs `{"BCShareAbsShockPct60": 0.81666667, "BCShareChange": 0.29645771}`; predicate TRUE.
  - FALSE example: ObservationDate `2023-11-14`; inputs `{"BCShareAbsShockPct60": null, "BCShareChange": 0.02585272}`; predicate FALSE.
...
- Required GEX source snapshot/equivalent: `GDX-20230815-20260813.csv`; columns `ObservationDate, ASXCode, GEXDelta, CapitalType, Close, VWAP`. Source database/schema: `NOT_AVAILABLE` from supplied artifacts; extraction identity is file id `14ASyKW8VxHXNPOHMcTsnpRW9T9zdnY-8` and exact file hash in manifest.
- Required outcome source/equivalent: `GDX-20251020-20260814.csv`; columns `TimeIntervalStart, Open, High, Low, Close, Volume, VWAP`.
...
|---|---|---:|---:|---:|---:|---:|---:|---|
| `GDX_CALL_BUY_SHARE_SURGE` | AVAILABLE | 30 | 21-9 | 70.0% | 2.26 | 2.48 | 2.19 | 52.1%–83.3% |
...
- Required source: `GDX` 30-minute or finer OHLC source equivalent to `2.19`.
...
- Title: `[GEX GDX] {signal_code} — {market_date}`. Body: `{direction} | {holding_period} | trigger inputs {key_values} | report {report_url}`.
...
- Report kind: `GEX_STRATEGY_EVALUATION`; title: `GDX GEX Strategy Evaluation — {market_date}`; subject/instrument `GDX`; contains strategy/version, source revision, every signal classification, exact decision inputs, trigger evaluations, entry/exit policy, historical-context reference and provenance.
