# AMD GEX Capital-Type Strategy Contract

## 1. Strategy identity

- **strategy_code:** `GEX_AMD_CAPITALTYPE`
- **display_name:** AMD GEX Capital-Type Strategy
- **version_code:** `1.0.0`
- **implementation_key:** `gex.amd.capital_type.v1_0_0`
- **status:** `REVIEW_READY`
- **purpose:** Use prior-session AMD option-GEX capital-type structure to classify next-session directional opportunities; runtime is notification-only.
- **economic/research rationale:** AMD weakness plus rising options fear inside an existing short-term decline historically behaves as multi-day exhaustion. A down day occurs while net bullish GEX positioning is extreme versus its own prior 60 observations. Price closes decisively above VWAP with elevated net-bullish and sell-put participation; historically strength tends to persist. A large collapse in bought-put share during a short-term decline historically anticipates a rebound. Options fear becomes extreme while price is compressed and refuses to break materially; retained as watch-only due to very small sample.
- **owner:** Wayne Cheng
...
- **source snapshot identifiers/hashes:** see `source-manifest.json`; combined source hash `34a10d737a8b89ae0647a3383240906f60dbb5179402d0a30751f6d7a84b3769`.
...
|---|---|---|---|---|---|---|---|
| SUBJECT | AMD | NASDAQ | USD | America/New_York | US market date of source observation | NASDAQ regular US equity calendar; exchange holidays skipped; America/New_York DST; early close uses last regular-session bar | LONG |
| SOURCE | AMD option GEX capital type | internal/Drive snapshot | USD-derived GEX | America/New_York | ObservationDate = completed US session | Previous US trading session is D0; four capital types BC/BP/SC/SP required | NONE |
| EXECUTION | AMD | NASDAQ | USD | America/New_York | D1/D2/... are US trading sessions after D0 | Exchange calendar; 07:30 premarket reference; early-close exit uses last regular bar | LONG |
...
|---:|---|---|---|---|---|---|---|
| 1 | `AMD_STRONG_FEAR_EXPANSION_REVERSAL` | `PriceChangePct < -0.10 AND PCRChangePct > 5.0 AND Momentum3Pct < -1.0` | LONG | PLAN_ENTRY | D1 07:30 ET bar Open; signal must be ready before/reference at that time | D5 final regular-session 30m bar Close; no TP/SL | yes |
| 2 | `AMD_BULLISH_POSITION_DIP_BUY` | `PriceChangePct < 0 AND NetBullSharePct60 >= 0.80` | LONG | PLAN_ENTRY | D1 07:30 ET bar Open; signal must be ready before/reference at that time | D5 final regular-session 30m bar Close; no TP/SL | yes |
| 3 | `AMD_SELL_PUT_SUPPORTED_STRENGTH` | `VWAPDiffPct > 0.15 AND NetBullSharePct60 >= 0.65 AND SPSharePct60 >= 0.65` | LONG | PLAN_ENTRY | D1 07:30 ET bar Open; signal must be ready before/reference at that time | D3 final regular-session 30m bar Close; no TP/SL | yes |
| 4 | `AMD_PUT_BUY_COLLAPSE_REBOUND` | `PutBuyShareChange < 0 AND PutBuyShareAbsShockPct60 >= 0.75 AND Momentum3Pct < -1.0` | LONG | PLAN_ENTRY | D1 07:30 ET bar Open; signal must be ready before/reference at that time | D2 final regular-session 30m bar Close; no TP/SL | yes |
| 5 | `AMD_COMPRESSION_SQUEEZE_WATCH` | `AbsPriceMovePct60 <= 0.30 AND PCRChangePct > 20.0 AND PCRAbsShockPct60 >= 0.75 AND NetBullSharePct60 <= 0.30 AND Close < VWAP` | LONG | WATCH | D1 07:30 ET bar Open; signal must be ready before/reference at that time | D1 final regular-session 30m bar Close; no TP/SL | no |
| 6 | `DATA_ERROR` | readiness passed but deterministic evaluation encounters impossible/non-finite derived state not caught by gate | NONE | NONE | none | none | no |
| 7 | `NO_SIGNAL` | no actionable/WATCH predicate true after readiness | NONE | NONE | none | none | no |
...
|---|---|---|---|---|---|---|---|
| Close | `AMD-20230922-20260813.csv` / `Close` | Observation-day underlying close | USD | raw; positive price | no rounding | NOT_READY if null | D0; current D0 included |
| VWAP | `AMD-20230922-20260813.csv` / `VWAP` | Observation-day source VWAP | USD | raw; positive price | no rounding | NOT_READY if null | D0; current D0 included |
| BCAbs | `AMD-20230922-20260813.csv` / `GEXDelta where CapitalType=BC` | ABS(GEXDelta) | GEX-delta units | absolute; nonnegative | no rounding | NOT_READY if missing | D0; current D0 included |
| BPAbs | `AMD-20230922-20260813.csv` / `GEXDelta where CapitalType=BP` | ABS(GEXDelta) | GEX-delta units | absolute; nonnegative | no rounding | NOT_READY if missing | D0; current D0 included |
| SCAbs | `AMD-20230922-20260813.csv` / `GEXDelta where CapitalType=SC` | ABS(GEXDelta) | GEX-delta units | absolute; nonnegative | no rounding | NOT_READY if missing | D0; current D0 included |
| SPAbs | `AMD-20230922-20260813.csv` / `GEXDelta where CapitalType=SP` | ABS(GEXDelta) | GEX-delta units | absolute; nonnegative | no rounding | NOT_READY if missing | D0; current D0 included |
...
- **AMD_STRONG_FEAR_EXPANSION_REVERSAL TRUE:** historical ObservationDate `2025-11-04`; relevant inputs `{"Momentum3Pct": -5.40233799, "PCRChangePct": 268.4524202, "PriceChangePct": -3.69728481}`; predicate TRUE.
  - FALSE example: ObservationDate `2023-12-21`; inputs `{"Momentum3Pct": 0.72714183, "PCRChangePct": 19.01420619, "PriceChangePct": 3.27747841}`; predicate FALSE.
...
- **AMD_BULLISH_POSITION_DIP_BUY TRUE:** historical ObservationDate `2025-11-06`; relevant inputs `{"NetBullSharePct60": 0.96666667, "PriceChangePct": -7.26797488}`; predicate TRUE.
  - FALSE example: ObservationDate `2023-12-20`; inputs `{"NetBullSharePct60": 0.25, "PriceChangePct": -3.33927934}`; predicate FALSE.
  - Missing example: any required trigger input = NULL => NOT_READY before evaluation, not NO_SIGNAL.
  - Stale example: latest complete ObservationDate older than expected previous US session => NOT_READY.
  - Tie/quantile example: percentile uses `<=`, so a current value equal to a prior value counts as less-than-or-equal; threshold comparison itself is inclusive when written `>=`/`<=`.
  - Boundary example: strict `>`/`<` thresholds do not trigger at equality; inclusive `>=`/`<=` thresholds do trigger at equality. Exact fixture supplied in `fixtures.json`.
- **AMD_SELL_PUT_SUPPORTED_STRENGTH TRUE:** historical ObservationDate `2025-11-21`; relevant inputs `{"NetBullSharePct60": 0.81666667, "SPSharePct60": 0.73333333, "VWAPDiffPct": 0.68380798}`; predicate TRUE.
  - FALSE example: ObservationDate `2023-12-20`; inputs `{"NetBullSharePct60": 0.25, "SPSharePct60": 0.16666667, "VWAPDiffPct": -1.66445272}`; predicate FALSE.
  - Missing example: any required trigger input = NULL => NOT_READY before evaluation, not NO_SIGNAL.
  - Stale example: latest complete ObservationDate older than expected previous US session => NOT_READY.
  - Tie/quantile example: percentile uses `<=`, so a current value equal to a prior value counts as less-than-or-equal; threshold comparison itself is inclusive when written `>=`/`<=`.
  - Boundary example: strict `>`/`<` thresholds do not trigger at equality; inclusive `>=`/`<=` thresholds do trigger at equality. Exact fixture supplied in `fixtures.json`.
- **AMD_PUT_BUY_COLLAPSE_REBOUND TRUE:** historical ObservationDate `2025-10-22`; relevant inputs `{"Momentum3Pct": -1.22275614, "PutBuyShareAbsShockPct60": 0.76666667, "PutBuyShareChange": -0.17090711}`; predicate TRUE.
  - FALSE example: ObservationDate `2023-12-20`; inputs `{"Momentum3Pct": -2.6446281, "PutBuyShareAbsShockPct60": null, "PutBuyShareChange": -0.01026834}`; predicate FALSE.
  - Missing example: any required trigger input = NULL => NOT_READY before evaluation, not NO_SIGNAL.
  - Stale example: latest complete ObservationDate older than expected previous US session => NOT_READY.
  - Tie/quantile example: percentile uses `<=`, so a current value equal to a prior value counts as less-than-or-equal; threshold comparison itself is inclusive when written `>=`/`<=`.
  - Boundary example: strict `>`/`<` thresholds do not trigger at equality; inclusive `>=`/`<=` thresholds do trigger at equality. Exact fixture supplied in `fixtures.json`.
- **AMD_COMPRESSION_SQUEEZE_WATCH TRUE:** historical ObservationDate `2025-10-28`; relevant inputs `{"AbsPriceMovePct60": 0.2, "Close": 258.01, "NetBullSharePct60": 0.1, "PCRAbsShockPct60": 0.86666667, "PCRChangePct": 104.12095315, "VWAP": 260.939}`; predicate TRUE.
  - FALSE example: ObservationDate `2023-12-20`; inputs `{"AbsPriceMovePct60": null, "Close": 135.47, "NetBullSharePct60": 0.25, "PCRAbsShockPct60": null, "PCRChangePct": 215.15163531, "VWAP": 137.763}`; predicate FALSE.
  - Missing example: any required trigger input = NULL => NOT_READY before evaluation, not NO_SIGNAL.
  - Stale example: latest complete ObservationDate older than expected previous US session => NOT_READY.
  - Tie/quantile example: percentile uses `<=`, so a current value equal to a prior value counts as less-than-or-equal; threshold comparison itself is inclusive when written `>=`/`<=`.
  - Boundary example: strict `>`/`<` thresholds do not trigger at equality; inclusive `>=`/`<=` thresholds do trigger at equality. Exact fixture supplied in `fixtures.json`.
...
- Required GEX source snapshot/equivalent: `AMD-20230922-20260813.csv`; columns `ObservationDate, ASXCode, GEXDelta, CapitalType, Close, VWAP`. Source database/schema: `NOT_AVAILABLE` from supplied artifacts; extraction identity is file id `1zBrQRlPhFATTxPbfKnXubYuFLLWEw3Fm` and exact file hash in manifest.
- Required outcome source/equivalent: `AMD-20251020-20260814.csv`; columns `TimeIntervalStart, Open, High, Low, Close, Volume, VWAP`.
...
|---|---|---:|---:|---:|---:|---:|---:|---|
| `AMD_STRONG_FEAR_EXPANSION_REVERSAL` | AVAILABLE | 17 | 15-2 | 88.2% | 6.32 | 6.51 | 27.30 | 65.7%–96.7% |
| `AMD_BULLISH_POSITION_DIP_BUY` | AVAILABLE | 18 | 15-3 | 83.3% | 4.55 | 6.91 | 3.88 | 60.8%–94.2% |
| `AMD_SELL_PUT_SUPPORTED_STRENGTH` | AVAILABLE | 20 | 17-3 | 85.0% | 3.51 | 3.18 | 6.87 | 64.0%–94.8% |
| `AMD_PUT_BUY_COLLAPSE_REBOUND` | AVAILABLE | 16 | 13-3 | 81.2% | 3.20 | 3.52 | 4.15 | 57.0%–93.4% |
| `AMD_COMPRESSION_SQUEEZE_WATCH` | INSUFFICIENT | 7 | 5-2 | 71.4% | 1.66 | 0.52 | 12.34 | 35.9%–91.8% |
...
- Required source: `AMD` 30-minute or finer OHLC source equivalent to `12.34`.
...
- Title: `[GEX AMD] {signal_code} — {market_date}`. Body: `{direction} | {holding_period} | trigger inputs {key_values} | report {report_url}`.
...
- Report kind: `GEX_STRATEGY_EVALUATION`; title: `AMD GEX Strategy Evaluation — {market_date}`; subject/instrument `AMD`; contains strategy/version, source revision, every signal classification, exact decision inputs, trigger evaluations, entry/exit policy, historical-context reference and provenance.
