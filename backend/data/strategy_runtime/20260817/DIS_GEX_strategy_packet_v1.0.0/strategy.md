# DIS GEX Capital-Type Strategy Contract

## 1. Strategy identity

- **strategy_code:** `GEX_DIS_CAPITALTYPE`
- **display_name:** DIS GEX Capital-Type Strategy
- **version_code:** `1.0.0`
- **implementation_key:** `gex.dis.capital_type.v1_0_0`
- **status:** `REVIEW_READY`
- **purpose:** Use prior-session DIS option-GEX capital-type structure to classify next-session directional opportunities; runtime is notification-only.
- **economic/research rationale:** DIS rallies with increasing PCR but unusually low total GEX participation; the rally historically lacks support and fades.
- **owner:** Wayne Cheng
...
- **research data range:** GEX 2023-09-26 through 2026-08-13; 30-minute execution data 2025-10-17 through 2026-08-14.
...
- **source snapshot identifiers/hashes:** see `source-manifest.json`; combined source hash `17926162fd0ba0e63ec3c42909f0db90bb2df06597e6300877145e063d8f6aff`.
...
|---|---|---|---|---|---|---|---|
| SUBJECT | DIS | NYSE | USD | America/New_York | US market date of source observation | NYSE regular US equity calendar; exchange holidays skipped; America/New_York DST; early close uses last regular-session bar | SHORT |
| SOURCE | DIS option GEX capital type | internal/Drive snapshot | USD-derived GEX | America/New_York | ObservationDate = completed US session | Previous US trading session is D0; four capital types BC/BP/SC/SP required | NONE |
| EXECUTION | DIS | NYSE | USD | America/New_York | D1/D2/... are US trading sessions after D0 | Exchange calendar; 07:30 premarket reference; early-close exit uses last regular bar | SHORT |
| BENCHMARK | SPY | NYSE Arca | USD | America/New_York | Same D1-to-Dn dates | Same US calendar alignment | NONE |
...
|---:|---|---|---|---|---|---|---|
| 1 | `DIS_LOW_GEX_RALLY_FEAR_FADE` | `PriceChangePct > 0.10 AND PCRChangePct > 5.0 AND TotalGEXPct60 <= 0.30` | SHORT | PLAN_ENTRY | D1 07:30 ET bar Open; signal must be ready before/reference at that time | D5 final regular-session 30m bar Close; no TP/SL | yes |
...
|---|---|---|---|---|---|---|---|
| Close | `DIS-20230922-20260813.csv` / `Close` | Observation-day underlying close | USD | raw; positive price | no rounding | NOT_READY if null | D0; current D0 included |
| VWAP | `DIS-20230922-20260813.csv` / `VWAP` | Observation-day source VWAP | USD | raw; positive price | no rounding | NOT_READY if null | D0; current D0 included |
| BCAbs | `DIS-20230922-20260813.csv` / `GEXDelta where CapitalType=BC` | ABS(GEXDelta) | GEX-delta units | absolute; nonnegative | no rounding | NOT_READY if missing | D0; current D0 included |
| BPAbs | `DIS-20230922-20260813.csv` / `GEXDelta where CapitalType=BP` | ABS(GEXDelta) | GEX-delta units | absolute; nonnegative | no rounding | NOT_READY if missing | D0; current D0 included |
| SCAbs | `DIS-20230922-20260813.csv` / `GEXDelta where CapitalType=SC` | ABS(GEXDelta) | GEX-delta units | absolute; nonnegative | no rounding | NOT_READY if missing | D0; current D0 included |
| SPAbs | `DIS-20230922-20260813.csv` / `GEXDelta where CapitalType=SP` | ABS(GEXDelta) | GEX-delta units | absolute; nonnegative | no rounding | NOT_READY if missing | D0; current D0 included |
...
- **DIS_LOW_GEX_RALLY_FEAR_FADE TRUE:** historical ObservationDate `2025-10-21`; relevant inputs `{"PCRChangePct": 36.02019447, "PriceChangePct": 2.09003215, "TotalGEXPct60": 0.1}`; predicate TRUE.
  - FALSE example: ObservationDate `2023-12-25`; inputs `{"PCRChangePct": 251.90189726, "PriceChangePct": null, "TotalGEXPct60": 0.36666667}`; predicate FALSE.
...
- Required GEX source snapshot/equivalent: `DIS-20230922-20260813.csv`; columns `ObservationDate, ASXCode, GEXDelta, CapitalType, Close, VWAP`. Source database/schema: `NOT_AVAILABLE` from supplied artifacts; extraction identity is file id `10_jjkHrWXJGlgN1SVsykSzgALYFT9h7P` and exact file hash in manifest.
- Required outcome source/equivalent: `DIS-20251017-20260814.csv`; columns `TimeIntervalStart, Open, High, Low, Close, Volume, VWAP`.
...
|---|---|---:|---:|---:|---:|---:|---:|---|
| `DIS_LOW_GEX_RALLY_FEAR_FADE` | AVAILABLE | 15 | 12-3 | 80.0% | 1.74 | 2.06 | 4.45 | 54.8%–93.0% |
...
- Required source: `DIS` 30-minute or finer OHLC source equivalent to `4.45`.
...
- Title: `[GEX DIS] {signal_code} — {market_date}`. Body: `{direction} | {holding_period} | trigger inputs {key_values} | report {report_url}`.
...
- Report kind: `GEX_STRATEGY_EVALUATION`; title: `DIS GEX Strategy Evaluation — {market_date}`; subject/instrument `DIS`; contains strategy/version, source revision, every signal classification, exact decision inputs, trigger evaluations, entry/exit policy, historical-context reference and provenance.
