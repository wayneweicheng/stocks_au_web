# ADBE GEX Capital-Type Strategy Contract

## 1. Strategy identity

- **strategy_code:** `GEX_ADBE_CAPITALTYPE`
- **display_name:** ADBE GEX Capital-Type Strategy
- **version_code:** `1.0.0`
- **implementation_key:** `gex.adbe.capital_type.v1_0_0`
- **status:** `REVIEW_READY`
- **purpose:** Use prior-session ADBE option-GEX capital-type structure to classify next-session directional opportunities; runtime is notification-only.
- **economic/research rationale:** ADBE GEX is both unusually concentrated and unusually net-bullish versus its prior 60 observations; the regime historically persists upward.
- **owner:** Wayne Cheng
...
- **research data range:** GEX 2023-09-25 through 2026-08-13; 30-minute execution data 2026-05-06 through 2026-08-14.
...
- **source snapshot identifiers/hashes:** see `source-manifest.json`; combined source hash `a93fb3fb1cd5c0d99d1a5f809438cbe61c6eea7f9b37d6e27cc5b0bb5efe54fd`.
...
|---|---|---|---|---|---|---|---|
| SUBJECT | ADBE | NASDAQ | USD | America/New_York | US market date of source observation | NASDAQ regular US equity calendar; exchange holidays skipped; America/New_York DST; early close uses last regular-session bar | LONG |
| SOURCE | ADBE option GEX capital type | internal/Drive snapshot | USD-derived GEX | America/New_York | ObservationDate = completed US session | Previous US trading session is D0; four capital types BC/BP/SC/SP required | NONE |
| EXECUTION | ADBE | NASDAQ | USD | America/New_York | D1/D2/... are US trading sessions after D0 | Exchange calendar; 07:30 premarket reference; early-close exit uses last regular bar | LONG |
...
|---:|---|---|---|---|---|---|---|
| 1 | `ADBE_CONCENTRATED_BULLISH_REGIME` | `GEXConcentrationPct60 >= 0.70 AND NetBullSharePct60 >= 0.70` | LONG | PLAN_ENTRY | D1 07:30 ET bar Open; signal must be ready before/reference at that time | D5 final regular-session 30m bar Close; no TP/SL | yes |
...
|---|---|---|---|---|---|---|---|
| Close | `ADBE-20230922-20260813.csv` / `Close` | Observation-day underlying close | USD | raw; positive price | no rounding | NOT_READY if null | D0; current D0 included |
| VWAP | `ADBE-20230922-20260813.csv` / `VWAP` | Observation-day source VWAP | USD | raw; positive price | no rounding | NOT_READY if null | D0; current D0 included |
| BCAbs | `ADBE-20230922-20260813.csv` / `GEXDelta where CapitalType=BC` | ABS(GEXDelta) | GEX-delta units | absolute; nonnegative | no rounding | NOT_READY if missing | D0; current D0 included |
| BPAbs | `ADBE-20230922-20260813.csv` / `GEXDelta where CapitalType=BP` | ABS(GEXDelta) | GEX-delta units | absolute; nonnegative | no rounding | NOT_READY if missing | D0; current D0 included |
| SCAbs | `ADBE-20230922-20260813.csv` / `GEXDelta where CapitalType=SC` | ABS(GEXDelta) | GEX-delta units | absolute; nonnegative | no rounding | NOT_READY if missing | D0; current D0 included |
| SPAbs | `ADBE-20230922-20260813.csv` / `GEXDelta where CapitalType=SP` | ABS(GEXDelta) | GEX-delta units | absolute; nonnegative | no rounding | NOT_READY if missing | D0; current D0 included |
...
- **ADBE_CONCENTRATED_BULLISH_REGIME TRUE:** historical ObservationDate `2026-05-14`; relevant inputs `{"GEXConcentrationPct60": 0.91666667, "NetBullSharePct60": 0.98333333}`; predicate TRUE.
  - FALSE example: ObservationDate `2023-12-21`; inputs `{"GEXConcentrationPct60": 0.38333333, "NetBullSharePct60": 0.63333333}`; predicate FALSE.
...
- Required GEX source snapshot/equivalent: `ADBE-20230922-20260813.csv`; columns `ObservationDate, ASXCode, GEXDelta, CapitalType, Close, VWAP`. Source database/schema: `NOT_AVAILABLE` from supplied artifacts; extraction identity is file id `1bNYx4T8lwVzrVQtaZlkPJrcKJGNqoJ_B` and exact file hash in manifest.
- Required outcome source/equivalent: `ADBE-20260506-20260814.csv`; columns `TimeIntervalStart, Open, High, Low, Close, Volume, VWAP`.
...
|---|---|---:|---:|---:|---:|---:|---:|---|
| `ADBE_CONCENTRATED_BULLISH_REGIME` | AVAILABLE | 18 | 13-5 | 72.2% | 2.53 | 4.28 | 2.58 | 49.1%–87.5% |
...
- Required source: `ADBE` 30-minute or finer OHLC source equivalent to `2.58`.
...
- Title: `[GEX ADBE] {signal_code} — {market_date}`. Body: `{direction} | {holding_period} | trigger inputs {key_values} | report {report_url}`.
...
- Report kind: `GEX_STRATEGY_EVALUATION`; title: `ADBE GEX Strategy Evaluation — {market_date}`; subject/instrument `ADBE`; contains strategy/version, source revision, every signal classification, exact decision inputs, trigger evaluations, entry/exit policy, historical-context reference and provenance.
