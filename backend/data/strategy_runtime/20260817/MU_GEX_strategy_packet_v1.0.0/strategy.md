# MU GEX Capital-Type Strategy Contract

## 1. Strategy identity

- **strategy_code:** `GEX_MU_CAPITALTYPE`
- **display_name:** MU GEX Capital-Type Strategy
- **version_code:** `1.0.0`
- **implementation_key:** `gex.mu.capital_type.v1_0_0`
- **status:** `REVIEW_READY`
- **purpose:** Use prior-session MU option-GEX capital-type structure to classify next-session directional opportunities; runtime is notification-only.
- **economic/research rationale:** Price declines while the all-four-capital-type bearish/bullish intent ratio improves materially; historically this behaves as a multi-day mean-reversion setup. A down day plus a large causal put/call-ratio shock is treated as fear exhaustion rather than continuation. A large collapse in call-buy share during an established short-term uptrend historically preceded renewed upside. A large bought-put-share surge while short-term momentum is already negative historically predicts one more session of downside.
- **owner:** Wayne Cheng
...
- **source snapshot identifiers/hashes:** see `source-manifest.json`; combined source hash `12fef13064300ae5b1624204a8f10db6bcc89cae866808a1b221c78028f912ae`.
...
|---|---|---|---|---|---|---|---|
| SUBJECT | MU | NASDAQ | USD | America/New_York | US market date of source observation | NASDAQ regular US equity calendar; exchange holidays skipped; America/New_York DST; early close uses last regular-session bar | LONG/SHORT |
| SOURCE | MU option GEX capital type | internal/Drive snapshot | USD-derived GEX | America/New_York | ObservationDate = completed US session | Previous US trading session is D0; four capital types BC/BP/SC/SP required | NONE |
| EXECUTION | MU | NASDAQ | USD | America/New_York | D1/D2/... are US trading sessions after D0 | Exchange calendar; 07:30 premarket reference; early-close exit uses last regular bar | LONG/SHORT |
...
|---:|---|---|---|---|---|---|---|
| 1 | `MU_DOWN_DAY_INTENT_REVERSAL` | `PriceChangePct < -0.10 AND IntentRatioChangePct < -10.0` | LONG | PLAN_ENTRY | D1 07:30 ET bar Open; signal must be ready before/reference at that time | D5 final regular-session 30m bar Close; no TP/SL | yes |
| 2 | `MU_FEAR_SHOCK_REVERSAL` | `PriceChangePct < -0.10 AND PCRChangePct > 5.0 AND PCRAbsShockPct60 >= 0.75` | LONG | PLAN_ENTRY | D1 07:30 ET bar Open; signal must be ready before/reference at that time | D5 final regular-session 30m bar Close; no TP/SL | yes |
| 3 | `MU_CALL_WASHOUT_UPTREND` | `CallBuyShareChange < 0 AND CallBuyShareAbsShockPct60 >= 0.75 AND Momentum3Pct > 1.0` | LONG | PLAN_ENTRY | D1 07:30 ET bar Open; signal must be ready before/reference at that time | D2 final regular-session 30m bar Close; no TP/SL | yes |
| 4 | `MU_PUT_BUY_MOMENTUM_BREAKDOWN` | `PutBuyShareChange > 0 AND PutBuyShareAbsShockPct60 >= 0.75 AND Momentum3Pct < -1.0` | SHORT | PLAN_ENTRY | D1 07:30 ET bar Open; signal must be ready before/reference at that time | D1 final regular-session 30m bar Close; no TP/SL | yes |
...
|---|---|---|---|---|---|---|---|
| Close | `MU-20230922-20260813.csv` / `Close` | Observation-day underlying close | USD | raw; positive price | no rounding | NOT_READY if null | D0; current D0 included |
| VWAP | `MU-20230922-20260813.csv` / `VWAP` | Observation-day source VWAP | USD | raw; positive price | no rounding | NOT_READY if null | D0; current D0 included |
| BCAbs | `MU-20230922-20260813.csv` / `GEXDelta where CapitalType=BC` | ABS(GEXDelta) | GEX-delta units | absolute; nonnegative | no rounding | NOT_READY if missing | D0; current D0 included |
| BPAbs | `MU-20230922-20260813.csv` / `GEXDelta where CapitalType=BP` | ABS(GEXDelta) | GEX-delta units | absolute; nonnegative | no rounding | NOT_READY if missing | D0; current D0 included |
| SCAbs | `MU-20230922-20260813.csv` / `GEXDelta where CapitalType=SC` | ABS(GEXDelta) | GEX-delta units | absolute; nonnegative | no rounding | NOT_READY if missing | D0; current D0 included |
| SPAbs | `MU-20230922-20260813.csv` / `GEXDelta where CapitalType=SP` | ABS(GEXDelta) | GEX-delta units | absolute; nonnegative | no rounding | NOT_READY if missing | D0; current D0 included |
...
- **MU_DOWN_DAY_INTENT_REVERSAL TRUE:** historical ObservationDate `2025-10-22`; relevant inputs `{"IntentRatioChangePct": -26.35909754, "PriceChangePct": -1.88837807}`; predicate TRUE.
  - FALSE example: ObservationDate `2023-12-26`; inputs `{"IntentRatioChangePct": -51.19276317, "PriceChangePct": 0.65903573}`; predicate FALSE.
...
- **MU_FEAR_SHOCK_REVERSAL TRUE:** historical ObservationDate `2025-11-17`; relevant inputs `{"PCRAbsShockPct60": 0.93333333, "PCRChangePct": 177.82680458, "PriceChangePct": -1.97706924}`; predicate TRUE.
  - FALSE example: ObservationDate `2023-12-26`; inputs `{"PCRAbsShockPct60": null, "PCRChangePct": -65.42987982, "PriceChangePct": 0.65903573}`; predicate FALSE.
  - Missing example: any required trigger input = NULL => NOT_READY before evaluation, not NO_SIGNAL.
  - Stale example: latest complete ObservationDate older than expected previous US session => NOT_READY.
  - Tie/quantile example: percentile uses `<=`, so a current value equal to a prior value counts as less-than-or-equal; threshold comparison itself is inclusive when written `>=`/`<=`.
  - Boundary example: strict `>`/`<` thresholds do not trigger at equality; inclusive `>=`/`<=` thresholds do trigger at equality. Exact fixture supplied in `fixtures.json`.
- **MU_CALL_WASHOUT_UPTREND TRUE:** historical ObservationDate `2025-11-05`; relevant inputs `{"CallBuyShareAbsShockPct60": 0.81666667, "CallBuyShareChange": -0.23811291, "Momentum3Pct": 6.1357644}`; predicate TRUE.
  - FALSE example: ObservationDate `2023-12-26`; inputs `{"CallBuyShareAbsShockPct60": null, "CallBuyShareChange": 0.0156852, "Momentum3Pct": 10.63667556}`; predicate FALSE.
  - Missing example: any required trigger input = NULL => NOT_READY before evaluation, not NO_SIGNAL.
  - Stale example: latest complete ObservationDate older than expected previous US session => NOT_READY.
  - Tie/quantile example: percentile uses `<=`, so a current value equal to a prior value counts as less-than-or-equal; threshold comparison itself is inclusive when written `>=`/`<=`.
  - Boundary example: strict `>`/`<` thresholds do not trigger at equality; inclusive `>=`/`<=` thresholds do trigger at equality. Exact fixture supplied in `fixtures.json`.
- **MU_PUT_BUY_MOMENTUM_BREAKDOWN TRUE:** historical ObservationDate `2025-11-13`; relevant inputs `{"Momentum3Pct": -6.45479668, "PutBuyShareAbsShockPct60": 0.9, "PutBuyShareChange": 0.14675998}`; predicate TRUE.
  - FALSE example: ObservationDate `2023-12-26`; inputs `{"Momentum3Pct": 10.63667556, "PutBuyShareAbsShockPct60": null, "PutBuyShareChange": -0.03287972}`; predicate FALSE.
  - Missing example: any required trigger input = NULL => NOT_READY before evaluation, not NO_SIGNAL.
  - Stale example: latest complete ObservationDate older than expected previous US session => NOT_READY.
  - Tie/quantile example: percentile uses `<=`, so a current value equal to a prior value counts as less-than-or-equal; threshold comparison itself is inclusive when written `>=`/`<=`.
  - Boundary example: strict `>`/`<` thresholds do not trigger at equality; inclusive `>=`/`<=` thresholds do trigger at equality. Exact fixture supplied in `fixtures.json`.
...
- Required GEX source snapshot/equivalent: `MU-20230922-20260813.csv`; columns `ObservationDate, ASXCode, GEXDelta, CapitalType, Close, VWAP`. Source database/schema: `NOT_AVAILABLE` from supplied artifacts; extraction identity is file id `1RMzOfbboTeRBwB5PrjaeOLYmMwOwLmoS` and exact file hash in manifest.
- Required outcome source/equivalent: `MU-20251020-20260814.csv`; columns `TimeIntervalStart, Open, High, Low, Close, Volume, VWAP`.
...
|---|---|---:|---:|---:|---:|---:|---:|---|
| `MU_DOWN_DAY_INTENT_REVERSAL` | AVAILABLE | 45 | 33-12 | 73.3% | 5.58 | 5.92 | 3.51 | 59.0%–84.0% |
| `MU_FEAR_SHOCK_REVERSAL` | AVAILABLE | 16 | 13-3 | 81.2% | 9.21 | 11.61 | 10.85 | 57.0%–93.4% |
| `MU_CALL_WASHOUT_UPTREND` | AVAILABLE | 17 | 14-3 | 82.4% | 3.91 | 3.89 | 6.76 | 59.0%–93.8% |
| `MU_PUT_BUY_MOMENTUM_BREAKDOWN` | AVAILABLE | 12 | 10-2 | 83.3% | 3.56 | 3.15 | 5.11 | 55.2%–95.3% |
...
- Required source: `MU` 30-minute or finer OHLC source equivalent to `5.11`.
...
- Title: `[GEX MU] {signal_code} — {market_date}`. Body: `{direction} | {holding_period} | trigger inputs {key_values} | report {report_url}`.
...
- Report kind: `GEX_STRATEGY_EVALUATION`; title: `MU GEX Strategy Evaluation — {market_date}`; subject/instrument `MU`; contains strategy/version, source revision, every signal classification, exact decision inputs, trigger evaluations, entry/exit policy, historical-context reference and provenance.
