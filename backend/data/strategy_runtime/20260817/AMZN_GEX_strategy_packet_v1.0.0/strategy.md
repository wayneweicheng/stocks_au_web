# AMZN GEX Capital-Type Strategy Contract

## 1. Strategy identity

- **strategy_code:** `GEX_AMZN_CAPITALTYPE`
- **display_name:** AMZN GEX Capital-Type Strategy
- **version_code:** `1.0.0`
- **implementation_key:** `gex.amzn.capital_type.v1_0_0`
- **status:** `REVIEW_READY`
- **purpose:** Use prior-session AMZN option-GEX capital-type structure to classify next-session directional opportunities; runtime is notification-only.
- **economic/research rationale:** A large causal increase in bought-put share historically behaves as capitulation/exhaustion and is followed by multi-day rebound.
- **owner:** Wayne Cheng
...
- **source snapshot identifiers/hashes:** see `source-manifest.json`; combined source hash `76b20e109dbe9824aea72a8492836240d32d996ea2c2d711cc5aef4905fa6012`.
...
|---|---|---|---|---|---|---|---|
| SUBJECT | AMZN | NASDAQ | USD | America/New_York | US market date of source observation | NASDAQ regular US equity calendar; exchange holidays skipped; America/New_York DST; early close uses last regular-session bar | LONG |
| SOURCE | AMZN option GEX capital type | internal/Drive snapshot | USD-derived GEX | America/New_York | ObservationDate = completed US session | Previous US trading session is D0; four capital types BC/BP/SC/SP required | NONE |
| EXECUTION | AMZN | NASDAQ | USD | America/New_York | D1/D2/... are US trading sessions after D0 | Exchange calendar; 07:30 premarket reference; early-close exit uses last regular bar | LONG |
...
|---:|---|---|---|---|---|---|---|
| 1 | `AMZN_BP_SHARE_SHOCK_REBOUND` | `BPShareChange > 0 AND BPShareAbsShockPct60 >= 0.75` | LONG | PLAN_ENTRY | D1 07:30 ET bar Open; signal must be ready before/reference at that time | D5 final regular-session 30m bar Close; no TP/SL | yes |
...
|---|---|---|---|---|---|---|---|
| Close | `AMZN-20230815-20260813.csv` / `Close` | Observation-day underlying close | USD | raw; positive price | no rounding | NOT_READY if null | D0; current D0 included |
| VWAP | `AMZN-20230815-20260813.csv` / `VWAP` | Observation-day source VWAP | USD | raw; positive price | no rounding | NOT_READY if null | D0; current D0 included |
| BCAbs | `AMZN-20230815-20260813.csv` / `GEXDelta where CapitalType=BC` | ABS(GEXDelta) | GEX-delta units | absolute; nonnegative | no rounding | NOT_READY if missing | D0; current D0 included |
| BPAbs | `AMZN-20230815-20260813.csv` / `GEXDelta where CapitalType=BP` | ABS(GEXDelta) | GEX-delta units | absolute; nonnegative | no rounding | NOT_READY if missing | D0; current D0 included |
| SCAbs | `AMZN-20230815-20260813.csv` / `GEXDelta where CapitalType=SC` | ABS(GEXDelta) | GEX-delta units | absolute; nonnegative | no rounding | NOT_READY if missing | D0; current D0 included |
| SPAbs | `AMZN-20230815-20260813.csv` / `GEXDelta where CapitalType=SP` | ABS(GEXDelta) | GEX-delta units | absolute; nonnegative | no rounding | NOT_READY if missing | D0; current D0 included |
...
- **AMZN_BP_SHARE_SHOCK_REBOUND TRUE:** historical ObservationDate `2025-11-03`; relevant inputs `{"BPShareAbsShockPct60": 0.91666667, "BPShareChange": 0.13498664}`; predicate TRUE.
  - FALSE example: ObservationDate `2023-11-09`; inputs `{"BPShareAbsShockPct60": null, "BPShareChange": 0.17814289}`; predicate FALSE.
...
- Required GEX source snapshot/equivalent: `AMZN-20230815-20260813.csv`; columns `ObservationDate, ASXCode, GEXDelta, CapitalType, Close, VWAP`. Source database/schema: `NOT_AVAILABLE` from supplied artifacts; extraction identity is file id `1Bu9UhW9qfyqoWtT5vuHku775ivZ1QIPd` and exact file hash in manifest.
- Required outcome source/equivalent: `AMZN-20251020-20260814.csv`; columns `TimeIntervalStart, Open, High, Low, Close, Volume, VWAP`.
...
|---|---|---:|---:|---:|---:|---:|---:|---|
| `AMZN_BP_SHARE_SHOCK_REBOUND` | AVAILABLE | 20 | 16-4 | 80.0% | 2.90 | 1.67 | 10.29 | 58.4%–91.9% |
...
- Required source: `AMZN` 30-minute or finer OHLC source equivalent to `10.29`.
...
- Title: `[GEX AMZN] {signal_code} — {market_date}`. Body: `{direction} | {holding_period} | trigger inputs {key_values} | report {report_url}`.
...
- Report kind: `GEX_STRATEGY_EVALUATION`; title: `AMZN GEX Strategy Evaluation — {market_date}`; subject/instrument `AMZN`; contains strategy/version, source revision, every signal classification, exact decision inputs, trigger evaluations, entry/exit policy, historical-context reference and provenance.
