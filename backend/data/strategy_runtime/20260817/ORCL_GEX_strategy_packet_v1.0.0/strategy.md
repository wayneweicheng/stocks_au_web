# ORCL GEX Capital-Type Strategy Contract

## 1. Strategy identity

- **strategy_code:** `GEX_ORCL_CAPITALTYPE`
- **display_name:** ORCL GEX Capital-Type Strategy
- **version_code:** `1.0.0`
- **implementation_key:** `gex.orcl.capital_type.v1_0_0`
- **status:** `REVIEW_READY`
- **purpose:** Use prior-session ORCL option-GEX capital-type structure to classify next-session directional opportunities; runtime is notification-only.
- **economic/research rationale:** Bullish GEX composition fails to keep ORCL above its observation-day VWAP; the failure historically precedes additional downside.
- **owner:** Wayne Cheng
...
- **source snapshot identifiers/hashes:** see `source-manifest.json`; combined source hash `5deb7941eddc6a75b90e6d3375c6ec98c75590bf553a0edda4a29d57d2f8966b`.
...
|---|---|---|---|---|---|---|---|
| SUBJECT | ORCL | NYSE | USD | America/New_York | US market date of source observation | NYSE regular US equity calendar; exchange holidays skipped; America/New_York DST; early close uses last regular-session bar | SHORT |
| SOURCE | ORCL option GEX capital type | internal/Drive snapshot | USD-derived GEX | America/New_York | ObservationDate = completed US session | Previous US trading session is D0; four capital types BC/BP/SC/SP required | NONE |
| EXECUTION | ORCL | NYSE | USD | America/New_York | D1/D2/... are US trading sessions after D0 | Exchange calendar; 07:30 premarket reference; early-close exit uses last regular bar | SHORT |
...
|---:|---|---|---|---|---|---|---|
| 1 | `ORCL_FAILED_BULLISH_SUPPORT` | `Close < VWAP AND BullSharePct60 >= 0.65` | SHORT | PLAN_ENTRY | D1 07:30 ET bar Open; signal must be ready before/reference at that time | D2 final regular-session 30m bar Close; no TP/SL | yes |
...
|---|---|---|---|---|---|---|---|
| Close | `ORCL-20230922-20260813.csv` / `Close` | Observation-day underlying close | USD | raw; positive price | no rounding | NOT_READY if null | D0; current D0 included |
| VWAP | `ORCL-20230922-20260813.csv` / `VWAP` | Observation-day source VWAP | USD | raw; positive price | no rounding | NOT_READY if null | D0; current D0 included |
| BCAbs | `ORCL-20230922-20260813.csv` / `GEXDelta where CapitalType=BC` | ABS(GEXDelta) | GEX-delta units | absolute; nonnegative | no rounding | NOT_READY if missing | D0; current D0 included |
| BPAbs | `ORCL-20230922-20260813.csv` / `GEXDelta where CapitalType=BP` | ABS(GEXDelta) | GEX-delta units | absolute; nonnegative | no rounding | NOT_READY if missing | D0; current D0 included |
| SCAbs | `ORCL-20230922-20260813.csv` / `GEXDelta where CapitalType=SC` | ABS(GEXDelta) | GEX-delta units | absolute; nonnegative | no rounding | NOT_READY if missing | D0; current D0 included |
| SPAbs | `ORCL-20230922-20260813.csv` / `GEXDelta where CapitalType=SP` | ABS(GEXDelta) | GEX-delta units | absolute; nonnegative | no rounding | NOT_READY if missing | D0; current D0 included |
...
- **ORCL_FAILED_BULLISH_SUPPORT TRUE:** historical ObservationDate `2025-10-20`; relevant inputs `{"BullSharePct60": 0.66666667, "Close": 277.18, "VWAP": 279.593}`; predicate TRUE.
  - FALSE example: ObservationDate `2023-12-29`; inputs `{"BullSharePct60": 0.35, "Close": 105.43, "VWAP": 105.36}`; predicate FALSE.
...
- Required GEX source snapshot/equivalent: `ORCL-20230922-20260813.csv`; columns `ObservationDate, ASXCode, GEXDelta, CapitalType, Close, VWAP`. Source database/schema: `NOT_AVAILABLE` from supplied artifacts; extraction identity is file id `1w-A_K1sNsoDirlVcQDXL1_JPwsy6N-5A` and exact file hash in manifest.
- Required outcome source/equivalent: `ORCL-20251020-20260814.csv`; columns `TimeIntervalStart, Open, High, Low, Close, Volume, VWAP`.
...
|---|---|---:|---:|---:|---:|---:|---:|---|
| `ORCL_FAILED_BULLISH_SUPPORT` | AVAILABLE | 34 | 28-6 | 82.4% | 2.45 | 1.98 | 5.96 | 66.5%–91.7% |
...
- Required source: `ORCL` 30-minute or finer OHLC source equivalent to `5.96`.
...
- Title: `[GEX ORCL] {signal_code} — {market_date}`. Body: `{direction} | {holding_period} | trigger inputs {key_values} | report {report_url}`.
...
- Report kind: `GEX_STRATEGY_EVALUATION`; title: `ORCL GEX Strategy Evaluation — {market_date}`; subject/instrument `ORCL`; contains strategy/version, source revision, every signal classification, exact decision inputs, trigger evaluations, entry/exit policy, historical-context reference and provenance.
