# VST GEX Capital-Type Strategy Contract

## 1. Strategy identity

- **strategy_code:** `GEX_VST_CAPITALTYPE`
- **display_name:** VST GEX Capital-Type Strategy
- **version_code:** `1.0.0`
- **implementation_key:** `gex.vst.capital_type.v1_0_0`
- **status:** `REVIEW_READY`
- **purpose:** Use prior-session VST option-GEX capital-type structure to classify next-session directional opportunities; runtime is notification-only.
- **economic/research rationale:** VST price and PCR both decline; historically downside tends to continue for one additional session. The same VST double-down condition that predicts D1 continuation historically mean-reverts by D5; this is intentionally a two-horizon co-emission.
- **owner:** Wayne Cheng
...
- **research data range:** GEX 2023-09-26 through 2026-08-13; 30-minute execution data 2026-05-11 through 2026-08-14.
...
- **source snapshot identifiers/hashes:** see `source-manifest.json`; combined source hash `576ce3c01f316c17f4a4c4b6fdcc1ebc015e16f2a62b676e7bda79751219ef8d`.
...
|---|---|---|---|---|---|---|---|
| SUBJECT | VST | NYSE | USD | America/New_York | US market date of source observation | NYSE regular US equity calendar; exchange holidays skipped; America/New_York DST; early close uses last regular-session bar | LONG/SHORT |
| SOURCE | VST option GEX capital type | internal/Drive snapshot | USD-derived GEX | America/New_York | ObservationDate = completed US session | Previous US trading session is D0; four capital types BC/BP/SC/SP required | NONE |
| EXECUTION | VST | NYSE | USD | America/New_York | D1/D2/... are US trading sessions after D0 | Exchange calendar; 07:30 premarket reference; early-close exit uses last regular bar | LONG/SHORT |
...
|---:|---|---|---|---|---|---|---|
| 1 | `VST_DOUBLE_DOWN_CONTINUATION_D1` | `PriceChangePct < -0.10 AND PCRChangePct < -5.0` | SHORT | PLAN_ENTRY | D1 07:30 ET bar Open; signal must be ready before/reference at that time | D1 final regular-session 30m bar Close; no TP/SL | yes |
| 2 | `VST_DOUBLE_DOWN_REVERSAL_D5` | `PriceChangePct < -0.10 AND PCRChangePct < -5.0` | LONG | PLAN_ENTRY | D1 07:30 ET bar Open; signal must be ready before/reference at that time | D5 final regular-session 30m bar Close; no TP/SL | yes |
| 3 | `DATA_ERROR` | readiness passed but deterministic evaluation encounters impossible/non-finite derived state not caught by gate | NONE | NONE | none | none | no |
| 4 | `NO_SIGNAL` | no actionable/WATCH predicate true after readiness | NONE | NONE | none | none | no |
...
|---|---|---|---|---|---|---|---|
| Close | `VST-20230922-20260813.csv` / `Close` | Observation-day underlying close | USD | raw; positive price | no rounding | NOT_READY if null | D0; current D0 included |
| VWAP | `VST-20230922-20260813.csv` / `VWAP` | Observation-day source VWAP | USD | raw; positive price | no rounding | NOT_READY if null | D0; current D0 included |
| BCAbs | `VST-20230922-20260813.csv` / `GEXDelta where CapitalType=BC` | ABS(GEXDelta) | GEX-delta units | absolute; nonnegative | no rounding | NOT_READY if missing | D0; current D0 included |
| BPAbs | `VST-20230922-20260813.csv` / `GEXDelta where CapitalType=BP` | ABS(GEXDelta) | GEX-delta units | absolute; nonnegative | no rounding | NOT_READY if missing | D0; current D0 included |
| SCAbs | `VST-20230922-20260813.csv` / `GEXDelta where CapitalType=SC` | ABS(GEXDelta) | GEX-delta units | absolute; nonnegative | no rounding | NOT_READY if missing | D0; current D0 included |
| SPAbs | `VST-20230922-20260813.csv` / `GEXDelta where CapitalType=SP` | ABS(GEXDelta) | GEX-delta units | absolute; nonnegative | no rounding | NOT_READY if missing | D0; current D0 included |
...
- **VST_DOUBLE_DOWN_CONTINUATION_D1 TRUE:** historical ObservationDate `2026-05-11`; relevant inputs `{"PCRChangePct": -52.3996441, "PriceChangePct": -1.23416694}`; predicate TRUE.
  - FALSE example: ObservationDate `2024-03-25`; inputs `{"PCRChangePct": -48.99277096, "PriceChangePct": 4.13924236}`; predicate FALSE.
...
- **VST_DOUBLE_DOWN_REVERSAL_D5 TRUE:** historical ObservationDate `2026-05-11`; relevant inputs `{"PCRChangePct": -52.3996441, "PriceChangePct": -1.23416694}`; predicate TRUE.
  - FALSE example: ObservationDate `2024-03-25`; inputs `{"PCRChangePct": -48.99277096, "PriceChangePct": 4.13924236}`; predicate FALSE.
  - Missing example: any required trigger input = NULL => NOT_READY before evaluation, not NO_SIGNAL.
  - Stale example: latest complete ObservationDate older than expected previous US session => NOT_READY.
  - Tie/quantile example: percentile uses `<=`, so a current value equal to a prior value counts as less-than-or-equal; threshold comparison itself is inclusive when written `>=`/`<=`.
...
- Required GEX source snapshot/equivalent: `VST-20230922-20260813.csv`; columns `ObservationDate, ASXCode, GEXDelta, CapitalType, Close, VWAP`. Source database/schema: `NOT_AVAILABLE` from supplied artifacts; extraction identity is file id `1KgaYYl9HmqxLf3eg_aJGZWv1uktFM7Sd` and exact file hash in manifest.
- Required outcome source/equivalent: `VST-20260511-20260814.csv`; columns `TimeIntervalStart, Open, High, Low, Close, Volume, VWAP`.
...
|---|---|---:|---:|---:|---:|---:|---:|---|
| `VST_DOUBLE_DOWN_CONTINUATION_D1` | AVAILABLE | 12 | 10-2 | 83.3% | 1.16 | 1.14 | 4.02 | 55.2%–95.3% |
| `VST_DOUBLE_DOWN_REVERSAL_D5` | AVAILABLE | 12 | 10-2 | 83.3% | 4.34 | 3.27 | 5.02 | 55.2%–95.3% |
...
- Required source: `VST` 30-minute or finer OHLC source equivalent to `5.02`.
...
- Title: `[GEX VST] {signal_code} — {market_date}`. Body: `{direction} | {holding_period} | trigger inputs {key_values} | report {report_url}`.
...
- Report kind: `GEX_STRATEGY_EVALUATION`; title: `VST GEX Strategy Evaluation — {market_date}`; subject/instrument `VST`; contains strategy/version, source revision, every signal classification, exact decision inputs, trigger evaluations, entry/exit policy, historical-context reference and provenance.
