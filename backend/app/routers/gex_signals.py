...
def get_gex_signals_for_day(
    ...
    try:
        ...
        where_exact = [
            f"[{date_col}] >= convert(date, ?)",
            f"[{date_col}] < dateadd(day, 1, convert(date, ?))",
        ]
        params_exact: List[Any] = [observation_date, observation_date]
        if code_col:
            where_exact.append(f"[{code_col}] = convert(varchar(20), ?)")
        ...
        for index, value in enumerate(params_exact_safe):
            if isinstance(value, date):
                params_exact_safe[index] = value.isoformat()
        ...
        where_fallback = [f"[{date_col}] < dateadd(day, 1, convert(date, ?))"]
        params_fallback: List[Any] = [observation_date]
        if code_col:
            where_fallback.append(f"[{code_col}] = convert(varchar(20), ?)")
        ...
        for index, value in enumerate(params_fallback_safe):
            if isinstance(value, date):
                params_fallback_safe[index] = value.isoformat()
        ...
        if code_col:
            ...
            where_like_exact_date = [
                f"[{date_col}] >= convert(date, ?)",
                f"[{date_col}] < dateadd(day, 1, convert(date, ?))",
                f"[{code_col}] LIKE convert(varchar(100), ?)",
            ]
            params_like_exact_date: List[Any] = [observation_date, observation_date, like_token]
            sql_like_exact_date = (
            ...
            for index, value in enumerate(params_like_exact_date_safe):
                if isinstance(value, date):
                    params_like_exact_date_safe[index] = value.isoformat()
            data_like_exact = model.execute_read_query(sql_like_exact_date, tuple(params_like_exact_date_safe)) or []
            if data_like_exact:
                return data_like_exact

            where_like_fallback_date = [
                f"[{date_col}] < dateadd(day, 1, convert(date, ?))",
                f"[{code_col}] LIKE convert(varchar(100), ?)",
            ]
            ...
            for index, value in enumerate(params_like_fallback_date_safe):
                if isinstance(value, date):
                    params_like_fallback_date_safe[index] = value.isoformat()
        ...
        sql_last_resort = (
            f"SELECT TOP (1) * FROM [{DB_NAME}].[{TABLE_SCHEMA}].[{TABLE_NAME}] "
            f"WHERE [{date_col}] < dateadd(day, 1, convert(date, ?)){order_sql}"
        )
