-- Stored procedure: [Stock].[usp_GetAlertUser]


--exec [Stock].[usp_GetAlertUser]
--@pvchASXCode = 'AVZ.AX',
--@pintAlertTypeID = 10


CREATE PROCEDURE [Stock].[usp_GetAlertUser]
@pbitDebug AS BIT = 0,
...
BEGIN --Proc
	...
	BEGIN TRY
		...
		select 
			...
			c.Email,
			c.Mobile as Mobile
		from Stock.UserAlertTypeMap as a
		...
		and c.IsDisabled = 0
		and a.AlertTypeID = @pintAlertTypeID
		union
		select 
			c.UserID,
			c.UserName,
			c.Email,
			c.Mobile as Mobile
		from Stock.StockUser as c
		where c.UserRole = 'Admin'
		union
		select
			b.UserID,
			b.UserName,
		...
		and @pintAlertTypeID = (select AlertTypeID from LookupRef.AlertType where AlertTypeName = 'Strategy Sell')
