-- Stored procedure: [HC].[usp_GetQualityPoster]

CREATE PROCEDURE [HC].[usp_GetQualityPoster]
@pbitDebug AS BIT = 0,
...
BEGIN --Proc
	...
	BEGIN TRY
		...
		inner join
		(
			select Poster, max(PostDateTime) as MaxCreateDate
			from HC.PostRaw
			where PosterIsHeart = 1
			group by Poster
		) as b
		on a.Poster = b.Poster
		where PosterType = 'DEFAULT'
		and Rating <= 12
		and UserID is not null
		and datediff(day, b.MaxCreateDate, getdate()) < 60
		--and Poster = 'BarrenBuffett'
		group by a.Poster
		order by Rating asc, a.Poster
