...
BEGIN --Proc
	...
	BEGIN TRY
		...
		select 
			...
			case when sum(case when ActualSellPrice != ActualBuyPrice then 1 else 0 end) > 0 then sum(case when ActualSellPrice > ActualBuyPrice then 1 else 0 end)*100.0/sum(case when ActualSellPrice != ActualBuyPrice then 1 else 0 end) else null end as TradeSuccessPercentage,
		...
		where ExecutionID = @pintExecutionID
		and isnull(ASXCode, '') = isnull(@pvchASXCode, '')

		insert into [BackTest].[BackReportSummary]
		(
			ExecutionID,
			ASXCode,
			[TotalTransaction],
