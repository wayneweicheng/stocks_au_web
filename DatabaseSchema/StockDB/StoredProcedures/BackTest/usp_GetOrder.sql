-- Stored procedure: [BackTest].[usp_GetOrder]


CREATE PROCEDURE [BackTest].[usp_GetOrder]
@pbitDebug AS BIT = 0,
...
AS
/******************************************************************************
File: usp_GetOrder.sql
Stored Procedure Name: usp_GetOrder
Overview
-----------------
Gets active backtest orders

MODIFIED: 2025-11-02 - Fixed hardcoded '5 mins' bug on line 136
          Now uses AS_BarCompletedInMin from view (dynamic bar sizes)
...
Example of use
-----------------
EXEC [BackTest].[usp_GetOrder]
    @pintOrderTypeID = 29,
    @pvchTradeAccountName = 'huanw2114',
    @pvchASXCode = NULL;
*******************************************************************************/
...
BEGIN --Proc
	...
	BEGIN TRY
		...
		--declare @pintOrderTypeID as int = 29
		...
		select distinct
		    ...
		    AS_Exit1Price,
		    AS_Exit2Price,
			-- FIXED: Use AS_BarCompletedInMin from view (was hardcoded '5 mins')
			AS_BarCompletedInMin,
			-- NEW: Include AS_BuyConditionType for DRAGONFLY vs SMA_UPTURN routing
			AS_BuyConditionType,
			OptionSymbol,
			OptionSymbolDetails,
			OptionBuySell
		...
		-- Any order that is older than 720 hours (30 days), we don't include it anymore.
		and datediff(hour, a.CreateDate, getdate()) < 720
		order by a.CreateDate desc

		select top 5000 *
		from #TempOrder
		--where ASXCode = 'META.US'
