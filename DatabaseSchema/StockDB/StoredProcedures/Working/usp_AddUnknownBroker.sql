-- Stored procedure: [Working].[usp_AddUnknownBroker]


CREATE PROCEDURE [Working].[usp_AddUnknownBroker]
@pbitDebug AS BIT = 0,
...
BEGIN --Proc
	...
	BEGIN TRY
		...
		select * from LookupRef.BrokerName
		where APIBrokerName = ''

		declare @vchAPIBrokerName as varchar(200) = 'Virtu ITG Aust.'
		
		insert into LookupRef.BrokerName
		(
			BrokerCode,
			BrokerName,
			BrokerDescr,
		...
		select
			'VirITG' as BrokerCode,
			@vchAPIBrokerName as BrokerName,
			null as BrokerDescr,
			getdate() as CreateDate,
			20 as BrokerLevel,
			@vchAPIBrokerName as APIBrokerName,
			1.00 as BrokerScore
