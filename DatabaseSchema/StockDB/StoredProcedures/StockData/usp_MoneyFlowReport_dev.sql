...
BEGIN --Proc
	...
	BEGIN TRY
		...
		(
			...
			from #TempCOS
			where ASXCode = a.ASXCode
			and cast(SaleDateTime as date) = a.ObservationDate
		)
		set identity_insert #TempCOS off
		...
		--	ASXCode,
		--	MarketDate,
		--	format(MoneyFlowAmount, 'N0') as MoneyFlowAmount,
		--	format(MoneyFlowAmountIn, 'N0') as MoneyFlowAmountIn,
		--	format(MoneyFlowAmountOut, 'N0') as MoneyFlowAmountOut,
		--	format(CumulativeMoneyFlowAmount, 'N0') as CumulativeMoneyFlowAmount,
		--	isnull(PriceChangePerc, 0) as PriceChangePerc,
		--	isnull(InPerc, 0) as InPerc,
		--	isnull(OutPerc, 0) as OutPerc,
		--	isnull(InNumTrades, 0) as InNumTrades,
		--	isnull(OutNumTrades, 0) as OutNumTrades,			
		--	isnull(InAvgSize, 0) as InAvgSize,
		--	isnull(OutAvgSize, 0) as OutAvgSize,
		--	isnull([Open], 0) as [Open],
		--	isnull([High], 0) as [High],
		--	isnull([Low], 0) as [Low],
		--	isnull([Close], 0) as [Close],
		--	isnull([VWAP], 0) as [VWAP],
