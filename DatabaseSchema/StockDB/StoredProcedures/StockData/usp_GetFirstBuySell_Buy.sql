-- Stored procedure: [StockData].[usp_GetFirstBuySell_Buy]




CREATE PROCEDURE [StockData].[usp_GetFirstBuySell_Buy]
@pbitDebug AS BIT = 0,
...
AS
/******************************************************************************
File: usp_GetFirstBuySell_Buy.sql
Stored Procedure Name: usp_GetFirstBuySell_Buy
Overview
-----------------
usp_GetFirstBuySell_Buy
...
BEGIN --Proc
	...
	BEGIN TRY
		...
		DECLARE @vchProcedureName AS VARCHAR(100);		SET @vchProcedureName = 'usp_GetFirstBuySell_Buy'
		...
		select 
			...
			isnull(c.[Close], d.Price) as [Close]
		into #TempStockList
		from 
		(
			select ASXCode 
		...
		and c.ObservationDate = @dtEnqDate
		left join 
		(
				select *, row_number() over (partition by ASXCode order by SaleDateTime desc) as SaleRank
				from StockData.CourseOfSaleSecondaryToday
				where ObservationDate = @dtEnqDate
		) as d
		on a.ASXCode = d.ASXCode
		and d.SaleRank = 1
		where isnull(c.[Close], 0.5) between 0.02 and 1
		and a.ASXCode not in ('14D.AX')
		--and a.ASXCode in ('PAR.AX')

		declare @vchStockList as varchar(max)

		select @vchStockList = '''' + STRING_AGG(cast(ASXCode as varchar(max)), ''', ''') + ''''
		from #TempStockList

		--SELECT @vchStockList = '''' + STRING_AGG(ASXCode, ''', ''') WITHIN GROUP (ORDER BY ASXCode) + ''''
		--FROM #TempStockList
		--OPTION (MAXDOP 1);
		...
		select 
			...
			b.[Close],
			row_number() over (partition by a.ASXCode, SaleDateTime order by case when Price is not null then 1 else 0 end desc, DateFrom desc) as RowNumber
		into #TempFirstBuySell	
		from StockData.v_StockTickSaleVsBidAsk_All as a with(nolock)
		left join #TempStockList as b
		on a.ASXCode = b.ASXCode
		where a.ASXCode is not null
		and a.ObservationDate = ''' + cast(@dtEnqDate as varchar(50)) + '''
		and a.ASXCode in (' + @vchStockList +')
		and cast(SaleDateTime as time) > ''10:15:00''
		and cast(SaleDateTime as time) < ''15:55:00''
