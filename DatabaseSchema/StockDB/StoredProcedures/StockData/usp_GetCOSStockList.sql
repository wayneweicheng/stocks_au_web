-- Stored procedure: [StockData].[usp_GetCOSStockList]





CREATE PROCEDURE [StockData].[usp_GetCOSStockList]
@pbitDebug AS BIT = 0,
@pintErrorNumber AS INT = 0 OUTPUT
AS
/******************************************************************************
File: usp_GetCOSStockList.sql
Stored Procedure Name: usp_GetCOSStockList
Overview
-----------------
usp_GetCOSStockList
...
BEGIN --Proc
	...
	BEGIN TRY
		...
		DECLARE @vchProcedureName AS VARCHAR(100);		SET @vchProcedureName = 'usp_GetCOSStockList'
		...
		(
			...
			--select
			--		a.ASXCode as ASXCode
			--		,'--**' + case when a.ASXCode is null then substring(a.ASXCode, 1, charindex('.', a.ASXCode, 0) - 1) else substring(a.ASXCode, 1, charindex('.', a.ASXCode, 0) - 1) + ' - ' + a.ASXCompanyName + isnull(' - ' + c.MarketCap, '') end as CompanyName
			...
			--(
			--	select 1
			--	from StockData.MarketDepth as x
