-- Stored procedure: [StockData].[usp_GetFirstBuySell]




CREATE PROCEDURE [StockData].[usp_GetFirstBuySell]
@pbitDebug AS BIT = 0,
...
BEGIN --Proc
	...
	BEGIN TRY
		...
		select 
			...
			as varchar(10)) as InstituteBuySellCode,
		...
		select 
			  ...
			  ,[ActBuySellInd]
			  --,[Exchange]
			  ,case when [DerivedInstitute] is null then ''
					when [DerivedInstitute] = 0 then 'R'
					when [DerivedInstitute] = 1 then 'I'
			   end as [DerivedInstitute]
			  ,InstituteBuySellCode
