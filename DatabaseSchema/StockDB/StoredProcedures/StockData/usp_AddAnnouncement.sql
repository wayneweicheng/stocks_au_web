...
@pbitIsAlreadySydneyTime as bit = 0  -- NEW PARAMETER: Set to 1 if datetime is already in Sydney timezone
AS
/******************************************************************************
...
@pbitIsAlreadySydneyTime -- Set to 1 if @pvchAnnDate and @pvchAnnTime are already in Sydney timezone (no UTC conversion needed)
...
Date:		2026-01-19
Author:		WAYNE CHENG
Description: Added @pbitIsAlreadySydneyTime parameter to handle datetime already in Sydney timezone
-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
*******************************************************************************/
...
BEGIN --Proc
	...
	BEGIN TRY
		...
		-- DateTime conversion logic - handles both UTC and Sydney timezone inputs
		declare @dtAnnDateTimeSydney as smalldatetime

		IF @pbitIsAlreadySydneyTime = 1
		BEGIN
			-- DateTime is already in Sydney timezone - no conversion needed
			-- Simply combine date and time and cast to smalldatetime
			SELECT @dtAnnDateTimeSydney = CAST(@pvchAnnDate + ' ' + LEFT(@pvchAnnTime, 8) AS smalldatetime)

			IF @pbitDebug = 1
			BEGIN
				PRINT 'DateTime already in Sydney timezone: ' + CAST(@dtAnnDateTimeSydney AS VARCHAR(50))
			END
		END
		ELSE
		BEGIN
			-- Legacy behavior: DateTime is in UTC - convert to Sydney timezone
			DECLARE @utcDateTime AS datetimeoffset = (SELECT CAST(@pvchAnnDate + ' ' + LEFT(@pvchAnnTime, 8) AS datetimeoffset) AT TIME ZONE 'UTC')
			SELECT @dtAnnDateTimeSydney = CAST(@utcDateTime AT TIME ZONE 'AUS Eastern Standard Time' AS smalldatetime)

			IF @pbitDebug = 1
			BEGIN
				PRINT 'DateTime converted from UTC to Sydney: ' + CAST(@dtAnnDateTimeSydney AS VARCHAR(50))
			END
		END

		declare @dtRetriveDate as smalldatetime
		select @dtRetriveDate = cast(@pvchAnnRetriveDateTime as smalldatetime)
		...
		(
			...
			and AnnDateTime = @dtAnnDateTimeSydney
		)
		begin
			...
			select
			  ...
			  ,@dtRetriveDate as [AnnRetriveDateTime]
			  ,@dtAnnDateTimeSydney as [AnnDateTime]
			  ...
			  ,cast(@dtAnnDateTimeSydney as date) ObservationDate
