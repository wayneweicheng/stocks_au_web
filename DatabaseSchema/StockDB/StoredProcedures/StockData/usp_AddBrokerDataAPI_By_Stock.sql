-- Stored procedure: [StockData].[usp_AddBrokerDataAPI_By_Stock]
...
CREATE PROCEDURE [StockData].[usp_AddBrokerDataAPI_By_Stock]
@pbitDebug AS BIT = 0,
@pintErrorNumber AS INT = 0 OUTPUT,
@pvchStockCode as varchar(10),
@pvchBrokerName as varchar(200),
...
BEGIN --Proc
	...
	BEGIN TRY
		...
		begin
			print('Please supply a valid stock code')
		end
		else
		begin
			if floor(@pdecNetVolume) = 0 and @pintBuyTradeCount = @pintSellTradeCount
			begin
				print('Inactive broker is skipped')
			end
			else
			begin
				delete a
				...
				select
				  ...
				  ,LEFT(@pvchStockCode, CHARINDEX('.', @pvchStockCode) - 1) as [Symbol]
