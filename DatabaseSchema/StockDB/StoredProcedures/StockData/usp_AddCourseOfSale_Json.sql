-- Stored procedure: [StockData].[usp_AddCourseOfSale_Json]
...
CREATE PROCEDURE [StockData].[usp_AddCourseOfSale_Json]
@pbitDebug AS BIT = 0,
...
AS
/******************************************************************************
File: usp_AddCourseOfSale_Json.sql
Stored Procedure Name: usp_AddCourseOfSale_Json
Overview
-----------------
...
BEGIN --Proc
	...
	BEGIN TRY
		...
		DECLARE @vchProcedureName AS VARCHAR(100);		SET @vchProcedureName = 'usp_AddCourseOfSale_Json'
		...
		--select @jsonCourseOfSale = RawData from StockData.RawData
		--where RawDataID = 9262
		...
		select 
			try_cast(replace(json_value(b.value, '$.Time'), ',', '') as varchar(200)) as [Time],
			try_cast(replace(json_value(b.value, '$.Quantity'), ',', '') as int) as Quantity,
