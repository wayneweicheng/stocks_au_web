...
BEGIN --Proc
	...
	BEGIN TRY
		...
		delete a
		from StockData.MonthlyCurrent as a

		dbcc checkident('StockData.MonthlyCurrent', reseed, 1);

		insert into StockData.MonthlyCurrent
		(
		   [ASXCode]
		  ,[ObservationDate]
		  ,[Close]
		  ...
		  ,[DateSeqReverse]
		  ,[Prev3MovingAverage5d]
		  ,[Prev2MovingAverage5d]
		  ,[Prev1MovingAverage5d]
		  ,[RowNumber]
		)
		select
		   [ASXCode]
		...
		delete a
		from StockData.WeeklyCurrent as a

		dbcc checkident('StockData.WeeklyCurrent', reseed, 1);

		insert into StockData.WeeklyCurrent
		(
		   [ASXCode]
		  ,[ObservationDate]
		  ,[Close]
		  ...
		  ,[Prev2MovingAverage5d]
		  ,[Prev1MovingAverage5d]
		  ,[Prev2MovingAverage10d]
		  ,[Prev1MovingAverage10d]
		  ,[RowNumber]
		)
		select
		   [ASXCode]
		...
		(
			select 
				...
				try_cast(mv.MedianTradeValue as int) as MedianTradeValueWeekly,
				try_cast(mv.MedianTradeValueDaily as int) as MedianTradeValueDaily,
