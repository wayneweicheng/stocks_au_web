-- Stored procedure: [StockData].[usp_GetFirstBuySell_BidVolumeChange]



CREATE PROCEDURE [StockData].[usp_GetFirstBuySell_BidVolumeChange]
@pbitDebug AS BIT = 0,
...
select 
	...
	row_number() over (partition by SaleDateTime order by case when Price is not null then 1 else 0 end desc, DateFrom desc) as RowNumber
...
select cast(ObservationDateTime as smalldatetime), avg(Buy1Price) as Buy1Price, format(sum(cast(replace(Bid1VolumeDelta, '','', '''') as int)), ''N0'') as BidChanges
from #TempPriceBidAsk
where Buy1Price < Sell1Price
and Buy1Price > 0
group by cast(ObservationDateTime as smalldatetime)
order by cast(ObservationDateTime as smalldatetime) desc;
		'
