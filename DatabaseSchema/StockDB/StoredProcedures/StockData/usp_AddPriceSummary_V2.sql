-- Stored procedure: [StockData].[usp_AddPriceSummary_V2]
...
CREATE PROCEDURE [StockData].[usp_AddPriceSummary_V2]
@pbitDebug AS BIT = 0,
@pintErrorNumber AS INT = 0 OUTPUT,
@pvchASXCode as varchar(10),
@pdtQuoteTime as datetime,
...
BEGIN --Proc
	...
	BEGIN TRY
		...
		DECLARE @vchProcedureName AS VARCHAR(100);		SET @vchProcedureName = 'usp_AddPriceSummary_V2'
		...
		--into MAWork.dbo.PriceSummaryToday
		...
		--	and x.PriceSummaryID = @intPriceSummaryID
		...
		--		x.Vwap = x.[Close]
		--	from StockData.PriceSummaryToday as x
		--	where Prev1PriceSummaryID is null

		--	update x
		--	set x.Volume = case when x.Volume <= 0 then x.Prev1Volume else x.Volume end
		--	from StockData.PriceSummaryToday as x
		--	where x.DateTo is null
		--	and x.ASXCode = @pvchASXCode
		--	and x.PriceSummaryID = @intPriceSummaryID
		...
		--	update x
		--	set x.Value = y.Value + (x.Volume - y.Volume)*x.[Close]
		--	from StockData.PriceSummaryToday as x
		--	left join StockData.PriceSummaryToday as y
		--	on x.Prev1PriceSummaryID = y.PriceSummaryID 
		--	and x.DateTo is null
		--	and x.ASXCode = @pvchASXCode
		--	and x.PriceSummaryID = @intPriceSummaryID
		...
		--	update x
		--	set x.VWAP = x.Value*1.0/x.Volume
		--	from StockData.PriceSummaryToday as x
		--	where x.DateTo is null
		--	and x.ASXCode = @pvchASXCode
		--	and x.PriceSummaryID = @intPriceSummaryID
