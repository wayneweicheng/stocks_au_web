...
BEGIN --Proc
	...
	BEGIN TRY
		...
		set 
			...
			a.MovingAverage3d = b.MovingAverage3d,
		...
		(
			select 
				...
				MovingAverage3d = avg([Close]) over (partition by ASXCode order by DateSeq asc rows 2 preceding),
		...
		dbcc checkident('Alert.StockStatsHistoryPlus', reseed, 1);
		...
		delete a
		from Alert.StockStatsHistoryPlusCurrent as a

		dbcc checkident('Alert.StockStatsHistoryPlusCurrent', reseed, 1);

		insert into Alert.StockStatsHistoryPlusCurrent
		(
		   [ASXCode]
		  ,[ObservationDate]
		  ,[Close]
