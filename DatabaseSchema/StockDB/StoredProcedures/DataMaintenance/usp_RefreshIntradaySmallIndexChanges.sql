-- Stored procedure: [DataMaintenance].[usp_RefreshIntradaySmallIndexChanges]





CREATE PROCEDURE [DataMaintenance].[usp_RefreshIntradaySmallIndexChanges]
@pbitDebug AS BIT = 0,
@pdtObservationDate as date = null,
@pdtObservationStartDate as date = null,
@pbitReprocess as bit = 0,
@pintErrorNumber AS INT = 0 OUTPUT
AS
/******************************************************************************
File: usp_RefreshIntradaySmallIndexChanges.sql
Stored Procedure Name: usp_RefreshIntradaySmallIndexChanges
Overview
-----------------
usp_RefreshIntradaySmallIndexChanges
...
BEGIN --Proc
	...
	BEGIN TRY
		...
		DECLARE @vchProcedureName AS VARCHAR(100);		SET @vchProcedureName = 'usp_RefreshIntradaySmallIndexChanges'
		...
		if @pdtObservationStartDate is null
		begin
			select @pdtObservationStartDate = cast(getdate() as date)
		end
		...
		begin
			if object_id(N'Tempdb.dbo.#TempIntradaySmallIndexChanges') is not null
				drop table #TempIntradaySmallIndexChanges

			select *
			into #TempIntradaySmallIndexChanges
			from StockData.IntradaySmallIndexChanges as a
			...
			begin
				...
				(
					select 1
					from #TempIntradaySmallIndexChanges as a
					where ObservationDate = @pdtObservationDate
				...
				begin
					...
					begin
						select @pdtObservationTime = dateadd(minute, 2, @pdtObservationTime) 
					end
					else
					begin
				...
				insert into StockData.IntradaySmallIndexChanges
				(
					ObservationDate,
					ObservationTime,
					VsYesterdayClose_Up,
			...
			delete a
			from StockData.IntradaySmallIndexChanges as a with(nolock)
