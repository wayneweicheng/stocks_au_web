...
BEGIN --Proc
	...
	BEGIN TRY
		...
		--Code goes here 		
		truncate table Transform.BrokerRetailNet

		insert into Transform.BrokerRetailNet
		(
		   [ASXCode]
		  ,[ObservationDate]
		  ,[BrokerRetailNet]
		...
		if object_id(N'Transform.InstituteRetailNet') is not null
			drop table Transform.InstituteRetailNet
		...
		select
			...
			sum(NetValue) as NetValue
		into Transform.InstituteRetailNet
		from StockData.BrokerReport as a
		inner join LookupRef.BrokerName as b
		on a.BrokerCode = b.BrokerCode
		where a.ObservationDate > dateadd(day, -180, getdate())
		...
		if object_id(N'Transform.BrokerRetailNetSector') is not null
			drop table Transform.BrokerRetailNetSector
		...
		--	end as BrokerRetailNet,
		--	sum(NetValue) as NetValue
		--into Transform.BrokerRetailNetSector
		--from StockData.BrokerReport as a
		--inner join LookupRef.BrokerName as b
		--on a.BrokerCode = b.BrokerCode
