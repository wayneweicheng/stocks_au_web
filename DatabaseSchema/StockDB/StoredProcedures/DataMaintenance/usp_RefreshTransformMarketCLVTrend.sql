...
BEGIN --Proc
	...
	BEGIN TRY
		...
		cast(null as decimal(20, 4)) as XAO,
		cast(null as decimal(10, 2)) as XAOChange,
		'XAO' as Sector
		...
		update a
		set a.XAOChange = case when b.PrevXAO > 0 then cast((a.XAO - b.PrevXAO)*100.0/b.PrevXAO as decimal(10, 2)) else null end
		from #TempMarketCLVTrend as a
		inner join
		(
			select lead(XAO) over (partition by MarketCap, Sector order by ObservationDate desc) as PrevXAO, ObservationDate, Sector, MarketCap
			from Transform.MarketCLVTrend 
		) as b
		on a.ObservationDate = b.ObservationDate
		and a.Sector = b.Sector
		...
		on a.ObservationDate = b.ObservationDate
		where a.ObservationDate >  dateadd(day, -10, getdate())
