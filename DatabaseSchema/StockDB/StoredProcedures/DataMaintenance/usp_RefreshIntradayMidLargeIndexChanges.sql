-- Stored procedure: [DataMaintenance].[usp_RefreshIntradayMidLargeIndexChanges]





CREATE PROCEDURE [DataMaintenance].[usp_RefreshIntradayMidLargeIndexChanges]
@pbitDebug AS BIT = 0,
@pintErrorNumber AS INT = 0 OUTPUT
AS
/******************************************************************************
File: usp_RefreshIntradayMidLargeIndexChanges.sql
Stored Procedure Name: usp_RefreshIntradayMidLargeIndexChanges
Overview
-----------------
usp_RefreshIntradayMidLargeIndexChanges
...
BEGIN --Proc
	...
	BEGIN TRY
		...
		DECLARE @vchProcedureName AS VARCHAR(100);		SET @vchProcedureName = 'usp_RefreshIntradayMidLargeIndexChanges'
		...
		begin
			...
			begin
				...
				delete a
				from StockData.IntradayMidLargeIndexChanges as a
				where ObservationDate = @pdtObservationDate
				and ObservationTime = @pdtObservationTime

				insert into StockData.IntradayMidLargeIndexChanges
				(
					ObservationDate,
					ObservationTime,
					VsYesterdayClose_Up,
			...
			delete a
			from StockData.IntradayMidLargeIndexChanges as a with(nolock)
