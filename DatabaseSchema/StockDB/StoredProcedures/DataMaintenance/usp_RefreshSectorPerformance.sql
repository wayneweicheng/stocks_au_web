...
BEGIN --Proc
	...
	BEGIN TRY
		...
		begin
			...
			inner join LookupRef.StockKeyToken as a 
			on a.Token = c.Token
			...
			dbcc checkident('Report.SectorPerformance', reseed, 1);

			insert into Report.SectorPerformance
			(
				Token, 
				ObservationDate, 
				HoldValue, 
			...
			delete a
			from Report.SectorPerformanceDetails as a

			dbcc checkident('Report.SectorPerformanceDetails', reseed, 1);

			insert into Report.SectorPerformanceDetails
			(
			   [Token]
			  ,[ASXCode]
			  ,[ObservationDate]
