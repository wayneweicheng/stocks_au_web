...
BEGIN --Proc
	...
	BEGIN TRY
		...
		delete a
		from StockData.DirectorBuyOnMarket as a

		dbcc checkident('StockData.DirectorBuyOnMarket', reseed, 1);
		
		insert into StockData.DirectorBuyOnMarket
		(
			[ASXCode],
			[CleansedMarketCap],
			[ValueConsideration],
		...
		delete a
		from StockData.SignificantHolder as a

		dbcc checkident('StockData.SignificantHolder', reseed, 1);
				
		insert into StockData.SignificantHolder
		(
			[ASXCode],
			[CleansedMarketCap],
			[AnnDescr],
