-- Stored procedure: [DataMaintenance].[usp_RefreshTransformMostTradedSmallCap]





CREATE PROCEDURE [DataMaintenance].[usp_RefreshTransformMostTradedSmallCap]
@pbitDebug AS BIT = 0,
...
BEGIN --Proc
	...
	BEGIN TRY
		...
		if object_id(N'[Transform].[MostTradedSmallCap]') is not null
			drop table [Transform].[MostTradedSmallCap]
		...
		select top 500
			...
			h.SurplusVolume,
			format(h.MatchVolume, 'N0') as MatchVolume,
			format(h.MatchVolume*h.Bid, 'N0') as MatchValue,
			h.Bid,
			h.Offer as Ask,
			...
			e.IndustrySubGroup,
			a.notes as Notes
		into [Transform].[MostTradedSmallCap]
		from (
			select 
				a.[ASXCode],
		...
		left hash join StockData.v_CompanyFloatingShare as b
		on a.ASXCode = b.ASXCode
		left join #TempCashVsMC as c
		on a.ASXCode = c.ASXCode
		--left join StockData.RetailParticipation as d
		--on a.ASXCode = d.ASXCode
		left hash join StockData.CompanyInfo as e with(nolock)
