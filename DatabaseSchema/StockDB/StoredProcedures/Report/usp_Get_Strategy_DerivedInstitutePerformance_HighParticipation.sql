-- Stored procedure: [Report].[usp_Get_Strategy_DerivedInstitutePerformance_HighParticipation]


CREATE PROCEDURE [Report].[usp_Get_Strategy_DerivedInstitutePerformance_HighParticipation]
@pbitDebug AS BIT = 0,
...
BEGIN --Proc
	...
	BEGIN TRY
		...
		DECLARE @vchProcedureName AS VARCHAR(100);		SET @vchProcedureName = 'usp_Get_Strategy_DerivedInstitutePerformance_HighParticipation'
		...
		begin
			...
			(
				...
				--or
				--(x.TradeValuePerc > 40 and x.TradeValuePerc > 1.0*x.AvgTradeValuePerc and a.TradeValuePerc > 40)
				x.InstituteBuyPerc > 1.3*x.RetailBuyPerc
			)
			--and 
			--(
			--	a.VWAP > b.VWAP
