-- Stored procedure: [Report].[usp_BuyCloseSellOpen]



CREATE PROCEDURE [Report].[usp_BuyCloseSellOpen]
@pbitDebug AS BIT = 0,
@pintErrorNumber AS INT = 0 OUTPUT,
@pintNumPrevDay as int = 0
AS
/******************************************************************************
File: usp_BuyCloseSellOpen.sql
Stored Procedure Name: usp_BuyCloseSellOpen
Overview
-----------------
usp_BuyCloseSellOpen
...
BEGIN --Proc
	...
	BEGIN TRY
		...
		DECLARE @vchProcedureName AS VARCHAR(100);		SET @vchProcedureName = 'usp_BuyCloseSellOpen'
		...
		select top 500
			'Buy Close Sell Open' as ReportType,
			...
			cast(IncreasePerc as decimal(10, 2)) as IncreasePerc, 
			cast(AvgOpenVsPrevClose as decimal(10, 2)) as AvgOpenVsPrevClose,
			g.[AnnDescr],
		...
		from (
			select 
				...
				AvgOpenVsPrevClose,
				null as Notes
			from StockData.BuyCloseSellOpen as a
		) as a
