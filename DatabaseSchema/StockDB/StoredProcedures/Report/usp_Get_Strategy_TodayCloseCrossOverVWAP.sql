-- Stored procedure: [Report].[usp_Get_Strategy_TodayCloseCrossOverVWAP]


CREATE PROCEDURE [Report].[usp_Get_Strategy_TodayCloseCrossOverVWAP]
@pbitDebug AS BIT = 0,
...
AS
/******************************************************************************
File: usp_Get_Strategy_TodayCloseCrossOverVWAP.sql
Stored Procedure Name: usp_Get_Strategy_TodayCloseCrossOverVWAP
Overview
-----------------
usp_Get_Strategy_TodayCloseCrossOverVWAP
...
BEGIN --Proc
	...
	BEGIN TRY
		...
		DECLARE @vchProcedureName AS VARCHAR(100);		SET @vchProcedureName = 'usp_Get_Strategy_TodayCloseCrossOverVWAP'
		...
		--select *
		--from #TempPriceSummaryLatest_DaySeq as a
		--inner join #TempPriceSummaryLatest_DaySeq as b
		--on a.ASXCode = b.ASXCode
		--and a.DaySeq = 0
		--and b.DaySeq = 1
		--inner join #TempPriceSummaryLatest_DaySeq as c
		--on a.ASXCode = c.ASXCode
		--and a.DaySeq = 0
		--and c.DaySeq = 2
		--inner join #TempPriceSummaryLatest_DaySeq as d
		...
		--and 
		--(
		--	(c.VWAP > b.VWAP and a.VWAP <= b.VWAP)
		--	or
		--	(d.VWAP > c.VWAP and c.VWAP > b.VWAP and a.VWAP > B.VWAP and d.VWAP > d.[Close])
		--)
		--and a.[Value] > 500000
		...
		--where x.ObservationDate = @dtObservationDate
		
		select distinct
			'Close above VWAP' as ReportType,
			null as [AnnDescr],
			a.ASXCode, 
		...
		from 
		(
			select a.ASXCode, a.[Close], a.[Value], a.Volume, a.[Open], a.[High], a.[Low]
			from #TempPriceSummaryLatest_DaySeq as a
			inner join #TempPriceSummaryLatest_DaySeq as b
			on a.ASXCode = b.ASXCode
			and a.DaySeq = 0
			...
			where a.[Close] >= Common.RoundStockPrice(a.VWAP)
			and Common.RoundStockPrice(b.VWAP) >= b.[Close]
			and Common.RoundStockPrice(c.VWAP) >= c.[Close]
			--and c.VWAP - b.VWAP > b.VWAP - a.VWAP
			and
			(
				Common.RoundStockPrice(a.VWAP) >= Common.RoundStockPrice(b.VWAP) or monitor.ASXCode is not null
			)
			and 
			(
				(c.VWAP > b.VWAP)
		...
		where 1 = 1
		and (a.[High] - a.[Open]) > 0 
		and cast(a.[Value]/1000.0 as int) > 100
		...
		order by 
			...
			cast(a.Volume*1.0/(f.FloatingShares*10000) as decimal(20, 2)) desc
