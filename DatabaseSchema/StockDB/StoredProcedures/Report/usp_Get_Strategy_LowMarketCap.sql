-- Stored procedure: [Report].[usp_Get_Strategy_LowMarketCap]


CREATE PROCEDURE [Report].[usp_Get_Strategy_LowMarketCap]
@pbitDebug AS BIT = 0,
...
BEGIN --Proc
	...
	BEGIN TRY
		...
		inner join Transform.CashVsMC as b
		...
		begin
			select distinct 
				'Low MarketCap Stocks' as ReportType,
				...
				format(plfm.Volume*plfm.[Close], 'N0') as TradeValue,
				plfm.MovingAverage20d,
			...
			into #TempResults
			from Transform.CashVsMC as cvm
			left join [StockData].[v_PriceSummary_Latest_Future_MA] as plfm
			on cvm.ASXCode = plfm.ASXCode
			and plfm.RowNumber = 2
			left join #TempDerivedInstitutePerformance as a
			on cvm.ASXCode = a.ASXCode
