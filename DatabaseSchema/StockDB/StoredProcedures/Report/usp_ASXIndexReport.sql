	...
	BEGIN TRY
		...
		(
			select 
				...
				b.PriceUp as PriceUpAll,
				b.PriceDown as PriceDownAll,
			...
			union
			select 
				'a. Close vs Last Close (Using IntraDay Price)' as ReportType,
				c.ObservationDate, 
				XAO.PriceChange as XAOPriceChange, 
				XJO.PriceChange as XJOPriceChange, 
				--b.PriceUp, 
			...
			on b.ObservationDate = c.ObservationDate
			inner join 
			(
				select ObservationDate,
					cast(sum(case when [Close] > PrevClose then 1 else 0 end)*100.0/count(*) as decimal(10, 2)) as PriceUp, 
			...
			union 
			select 
				'b. Close vs Open(Using IntraDay Price)' as ReportType,
				c.ObservationDate, 
				XAO.PriceChange as XAOPriceChange, 
				XJO.PriceChange as XJOPriceChange, 
				--b.PriceUp, 
