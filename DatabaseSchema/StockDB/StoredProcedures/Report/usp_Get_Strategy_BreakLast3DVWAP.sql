-- Stored procedure: [Report].[usp_Get_Strategy_BreakLast3DVWAP]


CREATE PROCEDURE [Report].[usp_Get_Strategy_BreakLast3DVWAP]
@pbitDebug AS BIT = 0,
...
AS
/******************************************************************************
File: usp_Get_Strategy_BreakLast3DVWAP.sql
Stored Procedure Name: usp_Get_Strategy_BreakLast3DVWAP
Overview
-----------------
usp_Get_Strategy_BreakLast3DVWAP
...
BEGIN --Proc
	...
	BEGIN TRY
		...
		DECLARE @vchProcedureName AS VARCHAR(100);		SET @vchProcedureName = 'usp_Get_Strategy_BreakLast3DVWAP'
		...
		select distinct
			'Break Last 3D VWAP' as ReportType,
			null as [AnnDescr],
			a.ASXCode, 
		...
		(
			...
			and a.[Close] > Common.RoundStockPrice(b.VWAP)
			and a.[Close] > Common.RoundStockPrice(c.VWAP)
			and a.[Close] > Common.RoundStockPrice(d.VWAP)
			--and Common.RoundStockPrice(b.VWAP) <= Common.RoundStockPrice(c.VWAP)
			and b.VWAP < c.VWAP
			and a.[Value] > 100000
		) as a
		...
		and a.[Close] < 20
		order by cast((a.[Close] - a.[Open])*100.0/(a.[High] - a.[Open]) as int) desc, cast(a.Volume*1.0/(f.FloatingShares*10000) as decimal(20, 2)) desc
