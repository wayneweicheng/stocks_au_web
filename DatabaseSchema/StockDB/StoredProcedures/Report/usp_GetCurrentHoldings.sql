-- Stored procedure: [Report].[usp_GetCurrentHoldings]



CREATE PROCEDURE [Report].[usp_GetCurrentHoldings]
@pbitDebug AS BIT = 0,
@pintErrorNumber AS INT = 0 OUTPUT
AS
/******************************************************************************
File: usp_GetCurrentHoldings.sql
Stored Procedure Name: usp_GetCurrentHoldings
Overview
-----------------
usp_GetCurrentHoldings
...
BEGIN --Proc
	...
	BEGIN TRY
		...
		DECLARE @vchProcedureName AS VARCHAR(100);		SET @vchProcedureName = 'usp_GetCurrentHoldings'
		...
		select top 500
			...
			a.ASXCode,
			case when h.IndicativePrice = 0 then h.[Close] else h.IndicativePrice end as [Close],
			format(h.SurplusVolume, 'N0') as SurplusVolume,
			h.Bid,
			h.Offer as Ask,
			...
			a.SourceSystem,
			a.AccountName,
			format(a.HeldVolume, 'N0') as HeldVolume,
			format(case when a.HeldVolume > 0 and case when h.Volume > 0 or h.IndicativePrice = 0 then h.[Close] else h.IndicativePrice end > 0 then a.HeldVolume*case when h.Volume > 0 or h.IndicativePrice = 0 then h.[Close] else h.IndicativePrice end end, 'N0') as HeldValue,
			g.[AnnDescr],
			...
			e.IndustrySubGroup
		from StockData.CurrentHoldings as a with(nolock)
		...
		order by 
			...
			case when a.SourceSystem = 'IB' then 1 
				 when a.SourceSystem = 'CMC' then 2
				 when a.SourceSystem = 'PEAK' then 3
				 else 99
			end asc,
			a.ASXCode
