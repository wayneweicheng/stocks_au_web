-- Stored procedure: [Report].[usp_Get_Strategy_DerivedInstitutePerformance_HighVWAP]


CREATE PROCEDURE [Report].[usp_Get_Strategy_DerivedInstitutePerformance_HighVWAP]
@pbitDebug AS BIT = 0,
...
AS
/******************************************************************************
File: sp_Get_Strategy_DerivedInstitutePerformance_HighVWAP.sql
Stored Procedure Name: sp_Get_Strategy_DerivedInstitutePerformance_HighVWAP
Overview
-----------------
sp_Get_Strategy_DerivedInstitutePerformance_HighVWAP
...
BEGIN --Proc
	...
	BEGIN TRY
		...
		DECLARE @vchProcedureName AS VARCHAR(100);		SET @vchProcedureName = 'sp_Get_Strategy_DerivedInstitutePerformance_HighVWAP'
		...
		declare @dtObservationDatePrevN as date = cast(Common.DateAddBusinessDay(-1 * @pintNumPrevDay - 4, getdate()) as date)
		...
		begin
			...
			where 
			(
				(x.TradeValuePerc > x.AvgTradeValuePerc*1.1 and x.TradeValuePerc > 40)
				and
				(x.InstituteBuyPerc > x.RetailBuyPerc*1.2 and x.InstituteBuyPerc > 50)
				and
				x.InstituteBuyVWAP > x.RetailBuyVWAP
			)
			--and 
			--(
			--	a.VWAP > b.VWAP
			...
			order by a.ObservationDate desc, format(x.TradeValuePerc, 'N1') desc, a.ASXCode desc
