-- Stored procedure: [Report].[usp_Get_Strategy_SignOfBullRun]


CREATE PROCEDURE [Report].[usp_Get_Strategy_SignOfBullRun]
@pbitDebug AS BIT = 0,
...
AS
/******************************************************************************
File: usp_Get_Strategy_SignOfBullRun.sql
Stored Procedure Name: usp_Get_Strategy_SignOfBullRun
Overview
-----------------
usp_Get_Strategy_SignOfBullRun
...
BEGIN --Proc
	...
	BEGIN TRY
		...
		DECLARE @vchProcedureName AS VARCHAR(100);		SET @vchProcedureName = 'usp_Get_Strategy_SignOfBullRun'
		...
		declare @dtObservationDatePrev10 as date = cast(Common.DateAddBusinessDay(-1 * @pintNumPrevDay - 10, getdate()) as date)
		...
		select *, cast(null as decimal(20, 4)) as PreviousDay_Close, row_number() over (partition by ASXCode order by DateFrom desc) as RowNumber
		into #TempPriceSummaryPrev1
		from StockData.v_PriceSummary
		where ObservationDate < @dtObservationDate
		and ObservationDate > @dtObservationDatePrev10
		and DateTo is null
		and [PrevClose] > 0
		and Volume > 0

		update a
		set a.PreviousDay_Close = b.[Close]
		from #TempPriceSummaryPrev1 as a
		...
		begin
			select 
				'Sign of bull run' as ReportType,
				a.Volume,
				a.ASXCode, 
				a.ObservationDate,
				cast(case when b.[Close] > 0 then (a.[Close] - b.[Close])*100.0/b.[Close] else null end as decimal(10, 2)) as PriceChange,
				m.BrokerCode as ObDateTopBuyBroker,
				n.BrokerCode as ObDateTopSellBroker,
			...
			left join #TempPriceSummaryPrev1 as b
			on a.ASXCode = b.ASXCode
			and a.[Close] > b.[High]
			and a.Volume > 1*b.Volume
			and b.RowNumber = 1
			and b.[Close] <= b.[Open]
			left join #TempPriceSummaryPrev1 as b1
			on a.ASXCode = b1.ASXCode
			and b1.Volume > b.Volume
			and b1.RowNumber = 2
			and b1.[Close] <= b1.[Open]
			left join #TempPriceSummaryPrev1 as b2
			on a.ASXCode = b2.ASXCode
			...
			and m.CurrBRDate = cast(@dtObservationDate as date)
			and m.NetBuySell = 'B'
			left join [Transform].[BrokerReportList] as n
			on a.ASXCode = n.ASXCode
			and n.LookBackNoDays = 0
			and n.CurrBRDate = cast(@dtObservationDate as date)
			and n.NetBuySell = 'S'
			...
			and b1.ASXCode is not null
			and exists
			(
				select 1
				from #TempPriceSummaryPrev1
				where ASXCode = a.ASXCode
				and RowNumber <= 8
				and case when PreviousDay_Close > 0 then ([Close] - PreviousDay_Close)/PreviousDay_Close else null end > 0.12 
				and [Value] > 500000
			)
			--and d.ASXCode is not null
			--and e.ASXCode is not null
			--and a.ASXCode = 'RGL.AX'
			and a.[Value] > 50000
			...
			--    end = 1
			order by case when (a.[High] - a.[Open]) > 0 then cast((a.[Close] - a.[Open])*100.0/(a.[High] - a.[Open]) as int) else null end desc
		end
		else
		begin
			...
			*
			...
			(
				select 
					'Breakaway Gap' as ReportType,
					a.Volume,
					a.ASXCode, 
					a.ObservationDate,
					case when b.[Close] > 0 then (a.[Close] - b.[Close])*100.0/b.[Close] else null end as PriceChange,
					m2.BrokerCode as RecentTopBuyBroker,
					n2.BrokerCode as RecentTopSellBroker,
					ttsu.FriendlyNameList,
					cast(coalesce(f.SharesIssued*a.[Close]*1.0, g.MC) as decimal(8, 2)) as MC,
				...
				inner join #TempPriceSummaryPrev1 as b
				on a.ASXCode = b.ASXCode
				and a.[Low] > b.[High]*1.05
				and a.Volume > 2*b.Volume
