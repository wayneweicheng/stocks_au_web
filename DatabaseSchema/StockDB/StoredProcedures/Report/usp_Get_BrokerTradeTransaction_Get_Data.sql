-- Stored procedure: [Report].[usp_Get_BrokerTradeTransaction_Get_Data]

CREATE PROCEDURE [Report].[usp_Get_BrokerTradeTransaction_Get_Data]
  ...
  BEGIN
      ...
      BEGIN TRY
          IF @pbitDebug = 1
          BEGIN
              PRINT 'Procedure: usp_Get_BrokerTradeTransaction_Get_Data'
              PRINT 'Parameters:'
          ...
          SELECT
              ...
              Market,
              CreateDate
          FROM stockDB.stockdata.brokertradetransaction btt
          WHERE ObservationDate >= @pStartDate
              AND ObservationDate <= @pEndDate
              AND (@pASXCodes IS NULL OR @pASXCodes = '' OR EXISTS (
                  SELECT 1 FROM #ASXCodeFilter WHERE ASXCode = btt.ASXCode
              ))
          ORDER BY ObservationDate, ASXCode, TransactionDateTime
