-- Stored procedure: [Report].[usp_GetASX300SectorTodayPerformance]


CREATE PROCEDURE [Report].[usp_GetASX300SectorTodayPerformance]
@pbitDebug AS BIT = 0,
	...
	BEGIN TRY
		...
		begin
			select
			  ...
			  ,[Close] as PrevClose
			  ,[VWAP]
			  ,case when cast(getdate() as time) between '8:10:00' and '10:10:00' then cast(replace(replace(MatchPriceChange, '%', ''), ',', '') as decimal(10, 2)) else ChangePerc end as ChangePerc
			  ,MatchPriceChange
			...
			(
				select 
				  ...
				  ,x.SearchTerm
				  ,cast(dense_rank() over (order by a.IndustryGroup) as decimal(5, 1)) as RankNumber
				...
				where isnull(a.[IndustryGroup], 'UNKNOWN') not in ('UNKNOWN')
				and a.ASX300 = 1
				union
				select 
				   'N/A' as Sector
