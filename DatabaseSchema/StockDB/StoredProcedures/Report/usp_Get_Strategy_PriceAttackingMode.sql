-- Stored procedure: [Report].[usp_Get_Strategy_PriceAttackingMode]


CREATE PROCEDURE [Report].[usp_Get_Strategy_PriceAttackingMode]
@pbitDebug AS BIT = 0,
...
AS
/******************************************************************************
File: usp_Get_Strategy_PriceAttackingMode.sql
Stored Procedure Name: usp_Get_Strategy_PriceAttackingMode
Overview
-----------------
usp_Get_Strategy_PriceAttackingMode
...
BEGIN --Proc
	...
	BEGIN TRY
		...
		DECLARE @vchProcedureName AS VARCHAR(100);		SET @vchProcedureName = 'usp_Get_Strategy_PriceAttackingMode'
		...
		begin
			select 
				'PriceAttackingMode' as ReportType,			
				a.Volume,
				a.ASXCode, 
				a.ObservationDate,
				...
				b.MovingAverage60d,
				d.ATR, 
				case when a.[High] = a.[Open] then null else cast((a.[Close] - a.[Open])*100.0/(a.[High] - a.[Open]) as int) end as BarStrength
			...
			and a.[Close] > b.MovingAverage20d
			...
			and b.MovingAverage20d > c.MovingAverage20d
			...
			and c.MovingAverage20d > c2.MovingAverage20d
			and b.MovingAverage5d > c2.MovingAverage5d + 1*d.ATR
			and b.MovingAverage10d > c2.MovingAverage10d + 0.5*d.ATR
