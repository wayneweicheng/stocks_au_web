-- Stored procedure: [Report].[usp_Get_Strategy_HeavyRetailSell]


CREATE PROCEDURE [Report].[usp_Get_Strategy_HeavyRetailSell]
@pbitDebug AS BIT = 0,
...
AS
/******************************************************************************
File: usp_Get_Strategy_HeavyRetailSell.sql
Stored Procedure Name: usp_Get_Strategy_HeavyRetailSell
Overview
-----------------
usp_Get_Strategy_HeavyRetailSell
...
Date:		2021-02-08
Author:		WAYNE CHENG
Description: usp_Get_Strategy_HeavyRetailSell
-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
*******************************************************************************/
...
BEGIN --Proc
	...
	BEGIN TRY
		...
		DECLARE @vchProcedureName AS VARCHAR(100);		SET @vchProcedureName = 'usp_Get_Strategy_HeavyRetailSell'
		...
		and BrokerCode in (
			...
			where BrokerScore >= 1.0
		)

		--declare @dtMaxBRDate as date = '2021-02-03'
		--declare @dtStartBRDate as date = '2021-02-03'

		if object_id(N'Tempdb.dbo.#TempBrokerDataSell') is not null
			drop table #TempBrokerDataSell

		--into #TempBrokerDataSell1
		select *
		...
		if object_id(N'Tempdb.dbo.#TempBrokerDataSellPlus') is not null
			drop table #TempBrokerDataSellPlus

		select *
		into #TempBrokerDataSellPlus
		...
		(
			...
			and BrokerCode in ('ComSec', 'CMCMar')
		...
		if object_id(N'Tempdb.dbo.#TempBrokerDataSellPlusList') is not null
			drop table #TempBrokerDataSellPlusList

		select *
		into #TempBrokerDataSellPlusList
		from
		(
			select distinct x.ASXCode, stuff((
				select top 2 ',' + [BrokerCode]
				from #TempBrokerDataSellPlus as a
				where x.ASXCode = a.ASXCode
				order by RowNumber asc
				for xml path('')), 1, 1, ''
			) as [BrokerCode], y.NetValue, y.NetVolume, y.SellBrokerRank
			from #TempBrokerDataSellPlus as x
			inner join
			(
				select ASXCode, sum(NetValue) as NetValue, sum(NetVolume) as NetVolume, '1, 2' as SellBrokerRank
				from #TempBrokerDataSellPlus
				group by ASXCode
			) as y
		...
		begin
			...
			inner join #TempBrokerDataSellPlusList as d
