-- Stored procedure: [Report].[usp_Get_Strategy_AdvancedHBXF]


CREATE PROCEDURE [Report].[usp_Get_Strategy_AdvancedHBXF]
@pbitDebug AS BIT = 0,
...
BEGIN --Proc
	...
	BEGIN TRY
		...
		--Code goes here 		
		--declare @pintNumPrevDay as int = 2
		...
		begin
			--declare @dtObservationDate as date = '2021-11-03'

			select
				'AdvancedHBXF' as ReportType,			
				a.Volume,
				a.ASXCode, 
				a.ObservationDate,
				cast((a.[Close] - a.PrevClose)*100.0/a.PrevClose as decimal(10, 2)) as PriceChange,
			...
			inner join #TempStockStatsHistoryPlus as b
			on a.ASXCode = b.ASXCode
			and b.RowNumber = 1
			inner join #TempStockStatsHistoryPlus as c
			on a.ASXCode = c.ASXCode
			and c.RowNumber = 2
			inner join #TempStockStatsHistoryPlus as c2
			on a.ASXCode = c2.ASXCode
			and c2.RowNumber = 3
			inner join #TempStockAvgTrueRange as d
			...
			and a.[Close] > b.MovingAverage60d
			and abs(b.MovingAverage5d - b.MovingAverage10d) < 1*d.ATR
			and b.MovingAverage5d > b.MovingAverage60d + 1*d.ATR
			and b.MovingAverage10d > b.MovingAverage60d + 1*d.ATR			
			and round(b.MovingAverage5d, 3) >= round(c.MovingAverage5d, 3)
			and round(b.MovingAverage10d, 3) >= round(c.MovingAverage10d, 3)
			and round(c.MovingAverage5d, 3) >= round(c2.MovingAverage5d, 3)
			and round(c.MovingAverage10d, 3) >= round(c2.MovingAverage10d, 3)
			and a.[High] - a.[Open] > 0
			and a.[Close] < 2
			and a.[Close] > a.[Open]
			and 
			(
				cast(a.[Value]/1000.0 as int) > 200
				or
				(cast(a.[Value]/1000.0 as int) > 50 and cast(a.[Value]/1000.0 as int) > 3*cast(j.MedianTradeValueDaily as int))
			)
			and not exists
			(
				select 1
				from Report.ReportResultHistory
				where ASXCode = a.ASXCode
				and ReportProc = OBJECT_SCHEMA_NAME(@@PROCID) + '.' + OBJECT_NAME(@@PROCID)
				and ObservationDate < a.ObservationDate
				and ObservationDate > Common.DateAddBusinessDay(-5, a.ObservationDate)
			)
			--and a.ASXCode = 'RLC.AX'
			and a.PrevClose > 0
			and cast((a.[Close] - a.PrevClose)*100.0/a.PrevClose as decimal(20, 4)) > 3.0
			order by 
				cast(a.[Value]/1000.0 as int)/isnull(cast(coalesce(f.SharesIssued*a.[Close]*1.0, g.MC) as decimal(8, 2)), 9999) desc, 
				abs(b.MovingAverage5d - b.MovingAverage60d)/d.ATR asc,
				cast((a.[Close] - a.[Open])*100.0/(a.[High] - a.[Open]) as int) desc
		end
		else
		begin
			--print 'skip'
			...
			*
			...
			(
				...
				and b.MovingAverage5d > b.MovingAverage60d + 1*d.ATR
				and b.MovingAverage10d > b.MovingAverage60d + 1*d.ATR			
				and b.MovingAverage5d > c.MovingAverage5d
				and b.MovingAverage10d > c.MovingAverage10d
				--and b.MovingAverage60d > c.MovingAverage60d + 0.1*d.ATR
				and c.MovingAverage5d > c2.MovingAverage5d
				and c.MovingAverage10d > c2.MovingAverage10d
				--and c.MovingAverage60d > c2.MovingAverage60d
				and a.[High] - a.[Open] > 0
