...
BEGIN --Proc
	...
	BEGIN TRY
		...
		select 
			...
			format(NetVolumeRatio, 'N0') as NetVolumeRatio,
			x.NoBuys,
			x.NoSells,
			x.Trades,
			cast(x.BuyPrice/100.0 as decimal(20,4)) as BuyPrice,
			cast(x.SellPrice/100.0 as decimal(20, 4)) SellPrice,
			x.ChiXVolOutOfTotal
		from
		(
			select 
				...
				NetVolume,
				TotalVolume, 
				cast(abs(NetVolume)*100.0/TotalVolume as decimal(10, 3)) as NetVolumeRatio,
				NoBuys,
				NoSells,
				Trades,
				BuyPrice,
			...
			select
				...
				-1*@intBRNetVolume as TotalVolume, 
				1 as NetVolumeRatio,
				null as NoBuys,
				null as NoSells,
				null as Trades,
			...
			select
				...
				null as SellVolume,
				null as NetVolume,
				@intBRVolume/2 - @intTotalVolume as TotalVolume, 
				null as NetVolumeRatio,
				null as NoBuys,
				null as NoSells,
				null as Trades,
				null as BuyPrice,
				null as SellPrice,				
				case when (@intBRVolume/2) > 0 then cast(((@intBRVolume/2) - @intTotalVolume)*100.0/(@intBRVolume/2) as decimal(10, 2)) else null end as ChiXVolOutOfTotal
		) as x
		inner join Transform.PriceHistory as y
