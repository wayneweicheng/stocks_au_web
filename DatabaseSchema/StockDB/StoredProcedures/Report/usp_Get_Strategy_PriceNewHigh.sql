-- Stored procedure: [Report].[usp_Get_Strategy_PriceNewHigh]


CREATE PROCEDURE [Report].[usp_Get_Strategy_PriceNewHigh]
@pbitDebug AS BIT = 0,
...
BEGIN --Proc
	...
	BEGIN TRY
		...
		declare @dtObservationDatePrevN as date = cast(Common.DateAddBusinessDay(-1 * @pintNumPrevDay -200, getdate()) as date)
		...
		begin
			...
			inner join
			(
				select ASXCode, max([Close]) as MaxClose 
				from #TempStockStatsHistoryPlus
				where RowNumber > 1
				group by ASXCode
			) as MaxClose
			on a.ASXCode = maxclose.ASXCode
			...
			and b.MovingAverage5d > b.MovingAverage60d
			and b.MovingAverage10d > b.MovingAverage60d
			and b.MovingAverage5d > c.MovingAverage5d
			and b.MovingAverage10d > c.MovingAverage10d
			and b.MovingAverage60d > c.MovingAverage60d
			and a.[High] - a.[Open] > 0
			...
			and (a.[Close] - maxclose.MaxClose) > 0.33*d.ATR
			and a.Volume > b.MovingAverage120dVol
			and cast(a.[Value]/1000.0 as int) > 500
			and not exists
			(
				select 1
				...
				and ObservationDate > Common.DateAddBusinessDay(-10, a.ObservationDate)
			)
			order by 
				abs(b.MovingAverage5d - b.MovingAverage60d)/d.ATR asc,
				cast(a.[Value]/1000.0 as int)/isnull(cast(coalesce(f.SharesIssued*a.[Close]*1.0, g.MC) as decimal(8, 2)), 9999) desc, cast((a.[Close] - a.[Open])*100.0/(a.[High] - a.[Open]) as int) desc
		end
		else
		begin
			--print 'skip'
