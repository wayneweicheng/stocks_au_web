-- View: [Transform].[v_SPXWOptionMoneyFlow]
...
CREATE view [Transform].[v_SPXWOptionMoneyFlow]
as
select 
	*,
...
(
	select 
		...
		case when b.ShortLongRatio > 1.01 and isnull(abs(cast(b.ShortMinusLongGEX*100.0/b.ShortGEX as decimal(10, 2))), 5) > 3.0 then 'Swing down' 
			 when b.ShortLongRatio < 0.99 and isnull(abs(cast(b.ShortMinusLongGEX*100.0/b.ShortGEX as decimal(10, 2))), 5) > 3.0 then 'Swing Up' 
		else 'No Signal'
