-- View: [Transform].[v_SPYOptionMoneyFlowHistory]





--select * from [Transform].[v_SPXOptionMoneyFlowHistory]
--order by ObservationDate desc, ArchiveDate desc;


CREATE view [Transform].[v_SPYOptionMoneyFlowHistory]
as
select 
	*,
...
(
	select 
		a.ASXCode, a.ObservationDate, a.ExpiryDate, a.ArchiveDate,
		...
		case when b.ShortLongRatio > a.ShortLongRatio*1.01 and b.ShortLongRatio > 1 and isnull(abs(cast(b.ShortMinusLongGEX*100.0/b.ShortGEX as decimal(10, 2))), 5) > 3.0 then 'Swing down' 
			 when b.ShortLongRatio > a.ShortLongRatio*0.99 and b.ShortLongRatio > 1.03 and isnull(abs(cast(b.ShortMinusLongGEX*100.0/b.ShortGEX as decimal(10, 2))), 5) > 3.0 then 'Swing down' 
			 ...
			 when b.ShortLongRatio < a.ShortLongRatio*1.01 and b.ShortLongRatio < 0.97 and isnull(abs(cast(b.ShortMinusLongGEX*100.0/b.ShortGEX as decimal(10, 2))), 5) > 3.0 then 'Swing Up' 
		else 'No Signal'
		end as SwingIndicator
	from
	(
		select * from StockDB_US.[Transform].[OptionTradeByExpiryDateHistory]
		where 1 = 1
		and MoneyType = 'Dump'
	) as a
	inner join
	...
	inner join 
	(
		select ASXCode, ObservationDate, ArchiveDate, min(ExpiryDate) as ExpiryDate from StockDB_US.[Transform].[OptionTradeByExpiryDateHistory]
		group by ASXCode, ObservationDate, ArchiveDate
	) as c
	on a.ObservationDate = c.ObservationDate
	and a.ExpiryDate = c.ExpiryDate
	and a.ASXCode = c.ASXCode
	and a.ArchiveDate = c.ArchiveDate
