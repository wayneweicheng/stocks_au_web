-- Stored procedure: [StockData].[usp_RefreshBuySellIndicatorPlus_Adhoc]



CREATE PROCEDURE [StockData].[usp_RefreshBuySellIndicatorPlus_Adhoc]
@pbitDebug AS BIT = 0,
...
Example of use
-----------------
exec [StockData].[usp_RefreshBuySellIndicatorPlus_Adhoc]
@pdtObservationDate = '2023-12-08'
...
BEGIN --Proc
	...
	BEGIN TRY
		...
		--declare @pdtObservationDate as date = '2023-12-08'
		if @pdtObservationDate is null
		begin
			select @pdtObservationDate = Common.DateAddBusinessDay(-1*1, getdate()) 
		end
		
		declare @dtObservationDate as date = @pdtObservationDate
		...
		(
			...
			where ASXCode not in ('SPXW.US', 'SPX.US', 'SPY.US', 'QQQ.US')
		) as x
		where 1 = 1
		...
		where a.ASXCode not in ('SPXW.US', 'SPY.US', 'QQQ.US', 'SPX.US')
		and ObservationDateLocal = @dtObservationDate
