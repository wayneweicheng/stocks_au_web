-- Stored procedure: [StockData].[usp_RefreshWatchList_CoreStock]



CREATE PROCEDURE [StockData].[usp_RefreshWatchList_CoreStock]
@pbitDebug AS BIT = 0,
@pintErrorNumber AS INT = 0 OUTPUT
AS
/******************************************************************************
File: usp_RefreshWatchList_CoreStock.sql
Stored Procedure Name: usp_RefreshWatchList_CoreStock
Overview
-----------------
usp_RefreshWatchList_CoreStock
...
BEGIN --Proc
	...
	BEGIN TRY
		...
		DECLARE @vchProcedureName AS VARCHAR(100);		SET @vchProcedureName = 'usp_RefreshWatchList_CoreStock'
		...
		update a
		set WatchListName = 'WL99'
		from [StockData].[PriceSummaryToday] as a
		inner join #TempStock as b
		on a.ASXCode = b.ASXCode
		where a.WatchListName != 'WL99'
