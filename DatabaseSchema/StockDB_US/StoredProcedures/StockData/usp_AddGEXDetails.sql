-- Stored procedure: [StockData].[usp_AddGEXDetails]


CREATE PROCEDURE [StockData].[usp_AddGEXDetails]
@pbitDebug AS BIT = 0,
...
BEGIN --Proc
	...
	BEGIN TRY
		...
		--Code goes here 
		if object_id(N'Tempdb.dbo.#TempGexDetails') is not null
			drop table #TempGexDetails
		...
		delete a
		from StockData.GEXDetails as a
		inner join #TempGexDetails as b
		on a.ASXCode = b.ASXCode
		and a.ObservationDate = b.ObservationDate

		insert into StockData.GEXDetails
		(
			ASXCode,
			ObservationDate,
			Response,
