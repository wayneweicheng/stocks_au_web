...
BEGIN --Proc
	...
	BEGIN TRY
		...
		--Code goes here 
		--begin transaction
		if object_id(N'Tempdb.dbo.#TempMajorStocks') is not null
			drop table #TempMajorStocks

		select top 1000 a.*
		into #TempMajorStocks
		from Stock.ASXCompany as a
		inner join StockDB_US.StockData.CompanyInfo as b
		on a.ASXCode = b.ASXCode
		where b.CleansedMarketCap > 0
		order by b.CleansedMarketCap desc;
		...
		select 
			'WL' + cast((b.CompanyID % 6 + 1) as varchar(10)) as WatchListName,
			a.ASXCode, 
		...
		truncate table StockData.WatchList
