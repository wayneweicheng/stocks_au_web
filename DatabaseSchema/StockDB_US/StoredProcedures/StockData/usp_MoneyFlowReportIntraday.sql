...
BEGIN --Proc
	...
	BEGIN TRY
		...
		--declare @pvchObservationDate date = '2019-12-24'
		--declare @pvchStockCode varchar(20) = 'KAR.AX'
		...
		(
			...
			(
				...
				from #TempUnknownCOS
				where ASXCode = a.ASXCode
				and cast(MinSaleDateTime as time) > cast('00:25:00' as time)
			)
			or
			not exists
			(
				...
				where ASXCode = a.ASXCode
				and cast(DateFrom as time) < cast('00:25:00' as time)
			)
		)
		and 
		(
			...
			(
				...
				from #TempUnknownCOS
				where ASXCode = a.ASXCode
				and cast(MaxSaleDateTime as time) < cast('08:50:00' as time)
			)
			or
			not exists
			(
				...
				from #TempPriceSummary
				where ASXCode = a.ASXCode
				and cast(DateFrom as time) > cast('08:50:00' as time)
			)
		)
		...
		(
			select 
				...
				--case when sum(Quantity) > 100 then sum(SaleValue)/sum(Quantity) else 0 end as VWAP
				avg(Price) as VWAP
			from #TempDetailPlusRaw as a
			where a.BuySellIndicator in ('Buy')
