-- Stored procedure: [StockData].[usp_AddMoneyFlowInfo_API]



CREATE PROCEDURE [StockData].[usp_AddMoneyFlowInfo_API]
@pbitDebug AS BIT = 0,
...
BEGIN --Proc
	...
	BEGIN TRY
		...
		select distinct
			ASXCode, 
			json_value([value], '$.label') as ObservationDate, 
			json_value([value], '$.long_short') as LongShort,
			json_value([value], '$.sentiment') as Sentiment,
			json_value([value], '$.rank') as MFRank,
			json_value([value], '$.total') as MFTotal,
			json_value([value], '$.near_score') as NearScore,
			json_value([value], '$.total_score') as TotalScore
		...
		select
			...
			(select max(ObservationDate) from #TempMoneyFlowInfoBasic) as MaxObservationDate
		from #TempMoneyFlowInfoBasic as a
