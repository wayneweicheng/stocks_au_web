-- Stored procedure: [StockData].[usp_AddMarketDepth_Json_Plus]
...
CREATE PROCEDURE [StockData].[usp_AddMarketDepth_Json_Plus]
@pbitDebug AS BIT = 0,
@pintErrorNumber AS INT = 0 OUTPUT,
@pvchMarketDepth as varchar(max),
@pvchUpdateMode as varchar(10),
@pvchCountryCode as varchar(10) = 'AX'
AS
/******************************************************************************
File: usp_AddMarketDepth_Json_Plus.sql
Stored Procedure Name: usp_AddMarketDepth_Json_Plus
Overview
-----------------
usp_AddMarketDepth_Json_Plus
...
BEGIN --Proc
	...
	BEGIN TRY
		...
		DECLARE @vchProcedureName AS VARCHAR(100);		SET @vchProcedureName = 'usp_AddMarketDepth_Json_Plus'
		...
		--@pxmlMarketDepth
		declare @pvchDateTime as varchar(100)
		...
		--where RawDataID = 13372

		if @pvchUpdateMode = 'Delta'
		begin
			select @jsonMarketDepth = [value]
			from openjson(@jsonMarketDepth)
			where [key] = 'd'
		end

		select @pvchDateTime = FormattedTime
		from openjson(@jsonMarketDepth)
		with
		(
			Quote nvarchar(max) as json
		) as QuoteDict
		cross apply openjson(QuoteDict.Quote)
		with
		(
			FormattedTime nvarchar(100)
		) as FormattedTime
		
		declare @vchModifiedDateTime as varchar(100) = convert(varchar(100), DATEADD(ms, DATEDIFF(ms, '00:00:00', @pvchDateTime), CONVERT(DATETIME, cast(getdate() as date))), 121)

		select @vchModifiedDateTime
		...
		select
			...
			dateadd(second, 1, convert(datetime, @vchModifiedDateTime, 121)) as SourceSystemDate
		...
		if object_id(N'Tempdb.dbo.#TempDepth') is not null
			drop table #TempDepth

		select 
			try_cast(json_value(b.value, '$.pT') as int) as DepthTypeID,
			try_cast(replace(json_value(b.value, '$.pN'), ',', '') as int) as NumTraders,
			try_cast(replace(json_value(b.value, '$.pV'), ',', '') as int) as volume,
			try_cast(replace(json_value(b.value, '$.pP'), ',', '') as decimal(20, 4)) as price,
			replace(json_value(b.value, '$.pU'), ',', '') as pU,
			replace(json_value(b.value, '$.pM'), ',', '') as pM
		into #TempDepth
		from openjson(@jsonMarketDepth) as a
		cross apply openjson(a.value) as b
		where a.[Key] = 'Changes'

		if object_id(N'MAWork.dbo.TempDepth') is not null
			drop table MAWork.dbo.TempDepth
		
		select *
		into MAWork.dbo.TempDepth
		from #TempDepth

		if @pvchUpdateMode = 'Full'
		begin
			print 'Get from Cache'
		end
		else
		begin
			if object_id(N'Tempdb.dbo.#TempCacheFull') is not null
				drop table #TempCacheFull
			...
			select
			  ...
			  ,[pM]
			into #TempCacheFull
			from StockData.CacheMarketDepth
			where ASXCode = @vchStockCode

			--delete price step
			delete a
			from #TempCacheFull as a
			inner join #TempDepth as b
			on a.price = b.price
			and a.DepthTypeID = b.DepthTypeID
			and b.NumTraders = 0

			--update price step
			update a
			set
			   [DepthTypeID] = b.DepthTypeID
			  ,[NumTraders] = b.NumTraders
			  ,[volume] = b.volume
			  ,[price] = b.price
			  ,[pU] = b.pU
			  ,[pM] = b.pM
			...
			on a.price = b.price
			and a.DepthTypeID = b.DepthTypeID
			where b.NumTraders > 0

			insert into #TempCacheFull
			(
			   [DepthTypeID]
			  ,[NumTraders]
			  ,[volume]
			...
			(
				...
				from #TempCacheFull
				where price = a.price
				and DepthTypeID = a.DepthTypeID
			)
			and a.NumTraders > 0

			truncate table #TempDepth

			insert into #TempDepth
			select * from #TempCacheFull
		end

		delete a
		from StockData.CacheMarketDepth as a
		where ASXCode = @vchStockCode

		insert into StockData.CacheMarketDepth
		(
			[ASXCode]
			,[DepthTypeID]
			,[NumTraders]
		...
		select *, cast(DENSE_RANK() over (order by price desc) as int) as Position
		into #TempBuyerList
		from #TempDepth
		where DepthTypeID = 0
		...
		select *, cast(DENSE_RANK() over (order by price asc) as int) as Position
		into #TempSellerList
		from #TempDepth
		where DepthTypeID = 1
		...
		update a
		set a.DateTo = dateadd(second, 1, convert(datetime, @vchModifiedDateTime, 121))
		...
		select
		  ...
		  ,price as [Price]
		  ,@vchStockCode as [ASXCode]
		  ,dateadd(second, 1, convert(datetime, @vchModifiedDateTime, 121)) as [DateFrom]
		  ,null as [DateTo]
		from #TempBuyerList as a
		where not exists
		(
		...
		declare @dtmFakeSaleDate as datetime = dateadd(second, 5, convert(datetime, @vchModifiedDateTime, 121))
