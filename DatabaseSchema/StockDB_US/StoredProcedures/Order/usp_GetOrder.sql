-- Stored procedure: [Order].[usp_GetOrder]



CREATE PROCEDURE [Order].[usp_GetOrder]
@pbitDebug AS BIT = 0,
...
BEGIN --Proc
	...
	BEGIN TRY
		...
		select
			...
			isnull([VolumeGt], 0) as [VolumeGt],
			[OrderVolume],
			OrderValue,
			ValidUntil,
			a.[CreateDate],
			[OrderTriggerDate]
		from [Order].[v_Order] as a
		inner join LookupRef.OrderType as b
		on b.IsDisabled = 0
		and a.OrderTypeID = b.OrderTypeID
		and a.OrderTypeID = @pintOrderTypeID
