-- Stored procedure: [HC].[usp_GetCommonStockPlus]

CREATE PROCEDURE [HC].[usp_GetCommonStockPlus]
@pbitDebug AS BIT = 0,
...
BEGIN --Proc
	...
	BEGIN TRY
		...
		DECLARE @vchProcedureName AS VARCHAR(100);		SET @vchProcedureName = 'usp_GetCommonStockPlus'
		...
		--Code goes here 
		--select top 1000 * from HC.PostRaw
		...
		group by ASXCode
		having count(distinct Poster) >= @pintMinPoster
		...
		begin
			select 
				...
				cast(c.MC as decimal(20,2)) as MC,
				cast(c.CashPosition as decimal(20,2)) CashPosition,
				...
				cast(isnull(h.[Value], e.[Value])/(cast(c.MC as decimal(20,2))*10000) as decimal(20,2)) as ValueOverMC,
				--f.NumPost1d,
		...
		begin
			...
			left join #TempPriceSummary as h
			on a.ASXCode = h.ASXCode
			order by isnull(cast(c.MC as decimal(20,2)), 99999), a.Poster
		end
		...
		begin
			...
			left join #TempPriceSummary as h
			on a.ASXCode = h.ASXCode
			order by LatestHoldDate desc, a.Poster
		end
		...
		begin
			...
			left join #TempPriceSummary as h
			on a.ASXCode = h.ASXCode
			order by cast(isnull(h.[Value], e.[Value])/(cast(c.MC as decimal(20,2))*10000) as decimal(20,2)) desc, a.ASXCode
		...
		begin
			...
			order by i.NumPoster desc, a.ASXCode
		...
		insert into HC.CommonStockPlusHistory
		(
			ASXCode,
			Poster,
			LatestHoldDate,
