-- Stored procedure: [DataMaintenance].[usp_RefreshMarketFlow_with_range]





CREATE PROCEDURE [DataMaintenance].[usp_RefreshMarketFlow_with_range]
@pbitDebug AS BIT = 0,
...
BEGIN --Proc
	...
	BEGIN TRY
		...
		--Code goes here
		DECLARE @NumberOfBackDays INT = 1;
		DECLARE @ErrorNumber INT = 0;

		WHILE @NumberOfBackDays <= 10
		BEGIN
			PRINT 'Running DataMaintenance.usp_RefreshMarketFlow for back days = '
				+ CAST(@NumberOfBackDays AS VARCHAR(10));

			SET @ErrorNumber = 0;

			EXEC DataMaintenance.usp_RefreshMarketFlow
				@pNumberOfBackDays = @NumberOfBackDays,
				@pintErrorNumber = @ErrorNumber OUTPUT;

			IF @ErrorNumber <> 0
			BEGIN
				PRINT 'Error occurred for back days = '
					+ CAST(@NumberOfBackDays AS VARCHAR(10))
					+ '. Error number = '
					+ CAST(@ErrorNumber AS VARCHAR(20));

				-- Stop the loop if one run fails
				BREAK;
			END

			SET @NumberOfBackDays = @NumberOfBackDays + 1;
		END
