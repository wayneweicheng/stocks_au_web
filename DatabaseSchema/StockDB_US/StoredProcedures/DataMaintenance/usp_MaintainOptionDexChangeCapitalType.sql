-- Stored procedure: [DataMaintenance].[usp_MaintainOptionDexChangeCapitalType]
...
CREATE PROCEDURE [DataMaintenance].[usp_MaintainOptionDexChangeCapitalType]
@pbitDebug AS BIT = 0,
@pintErrorNumber AS INT = 0 OUTPUT,
@pintPrevNumDay as int = 2
AS
/******************************************************************************
File: usp_MaintainOptionDexChangeCapitalType.sql
Stored Procedure Name: usp_MaintainOptionDexChangeCapitalType
Overview
-----------------
usp_MaintainOptionDexChangeCapitalType
...
BEGIN --Proc
	...
	BEGIN TRY
		...
		DECLARE @vchProcedureName AS VARCHAR(100);		SET @vchProcedureName = 'usp_MaintainOptionDexChangeCapitalType'
		...
		if object_id(N'Tempdb.dbo.#TempOptionDEXChange') is not null
			drop table #TempOptionDEXChange
			
		select y.*, x.[Close], x.VWAP
		into #TempOptionDEXChange
		...
		(
			select 
				...
				cast(sum(case when PorC = 'C' then 1 else -1 end*Delta*100*(OpenInterest - Prev1OpenInterest)) as int) as DeltaDelta 
		...
		delete a
		from [Transform].[OptionDEXChangeCapitalType] as a
		inner join 
		(
			select distinct ObservationDate, ASXCode
			from #TempOptionDEXChange
		) as b
		on a.ObservationDate = b.ObservationDate
		and a.ASXCode = b.ASXCode

		insert into [Transform].[OptionDEXChangeCapitalType]
		(
			[ObservationDate],
			[ASXCode],
			[DEXDelta],
		...
		from (
			select 
				...
				ObservationDate,
				PorC,
				case when PorC = 'C' then 1 else -1 end*cast(sum(a.Delta*a.OpenInterest*100) as bigint) as Delta
		...
		insert into Transform.OptionDEXByPutCall
		select * from #TempPC
