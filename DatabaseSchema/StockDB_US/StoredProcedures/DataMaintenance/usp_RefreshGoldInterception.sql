-- Stored procedure: [DataMaintenance].[usp_RefreshGoldInterception]



CREATE PROCEDURE [DataMaintenance].[usp_RefreshGoldInterception]
@pbitDebug AS BIT = 0,
@pintErrorNumber AS INT = 0 OUTPUT
AS
/******************************************************************************
File: usp_RefreshGoldInterception.sql
Stored Procedure Name: usp_RefreshGoldInterception
Overview
-----------------
usp_RefreshGoldInterception
...
BEGIN --Proc
	...
	BEGIN TRY
		...
		DECLARE @vchProcedureName AS VARCHAR(100);		SET @vchProcedureName = 'usp_RefreshGoldInterception'
		...
		if object_id(N'StockData.GoldInterception') is not null
			drop table StockData.GoldInterception

		select cast(getdate() as date) as ObservationDate, 'GoldIntercept' as ReportType, a.ASXCode, cast(getdate() as date) as MarketDate, a.AnnDescr, a.AnnDateTime, b.MC, b.CashPosition, DA_Utility.dbo.RegexMatch(AnnContent, '[0-9.]+\s{0,3}m\s{0,3}@\s{0,3}[0-9.]+\s{0,3}[g|G]\s{0,3}\/\s{0,3}t\s{0,3}(au|AU|Au)') as MatchText
		into StockData.GoldInterception
		from StockData.Announcement as a
		left join #TempCashVsMC as b
		on a.ASXCode = b.ASXCode
		where DA_Utility.dbo.RegexMatch(AnnContent, '[0-9.]+\s{0,3}m\s{0,3}@\s{0,3}[0-9.]+\s{0,3}[g|G]\s{0,3}\/\s{0,3}t\s{0,3}(au|AU|Au)') is not null
