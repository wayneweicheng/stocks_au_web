...
BEGIN --Proc
	...
	BEGIN TRY
		...
		--where Common.DateAddBusinessDay(-1 * 90, @dtObservationDate) < WeekOpenDate
		where dateadd(day, -120, @dtObservationDate) < WeekOpenDate		
		and NumTradeDay >= 4
		...
		where dateadd(day, -30, @dtObservationDate) < ObservationDate
		and ASXCode not in ('NASDAQ', 'DJIA', 'SPX')
		and Volume > 0
		order by MedianTradeValue desc
		...
		--where Common.DateAddBusinessDay(-1 * 90, @dtObservationDate) < ObservationDate
		where dateadd(day, -30, @dtObservationDate) < ObservationDate
		and Volume > 0
		and [Close] > 0
		...
		select 
			a.*, 
			b.CleansedMarketCap
		from #TempMedianTradeValueDaily as a
