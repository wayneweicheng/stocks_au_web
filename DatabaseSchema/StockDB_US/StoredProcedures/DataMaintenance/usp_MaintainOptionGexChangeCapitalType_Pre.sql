-- Stored procedure: [DataMaintenance].[usp_MaintainOptionGexChangeCapitalType_Pre]
...
CREATE PROCEDURE [DataMaintenance].[usp_MaintainOptionGexChangeCapitalType_Pre]
@pbitDebug AS BIT = 0,
...
BEGIN --Proc
	...
	BEGIN TRY
		...
		where ObservationDate >= @dtObservationDateMinusN --@dtObservationDate

		update a
		set a.OpenInterest = a.Prev1OpenInterest + case when a.OpenInterest > a.Prev1OpenInterest then 1 else -1 end*Prev1Volume
		--set a.OpenInterest = a.Prev1OpenInterest
		from #TempOptionDelayedQuote_All_V2 as a
		where Prev1Volume < abs(OpenInterest - Prev1OpenInterest)
		and Volume > 0
		...
		delete a
		from Transform.OptionGEXChangeCapitalType_Pre as a
		inner join 
		(
		...
		insert into Transform.OptionGEXChangeCapitalType_Pre
		(
			[ObservationDate],
			[ASXCode],
			[GEXDelta],
