-- Stored procedure: [DataMaintenance].[usp_ArchivePriceHistorySecondary]





CREATE PROCEDURE [DataMaintenance].[usp_ArchivePriceHistorySecondary]
@pbitDebug AS BIT = 0,
...
BEGIN --Proc
	...
	BEGIN TRY
		...
		DECLARE @vchProcedureName AS VARCHAR(100);		SET @vchProcedureName = 'usp_ArchivePriceHistorySecondary'
		...
		and CreateDate < dateadd(hour, -25, getdate())

		delete a
		from [StockData].[CourseOfSaleSecondaryToday] as a
		where CreateDate < dateadd(hour, -25, getdate())
