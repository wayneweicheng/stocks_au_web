...
BEGIN --Proc
	...
	BEGIN TRY
		...
		truncate table ScanResults.StockStatsHistoryPlus
		...
		if object_id(N'ScanResults.StockStatsHistoryPlusCurrent') is not null
			drop table ScanResults.StockStatsHistoryPlusCurrent

		select
			a.*
		into ScanResults.StockStatsHistoryPlusCurrent
		...
		create clustered index idx_scanresultsstockstatshistorypluscurrent_asxcode on ScanResults.StockStatsHistoryPlusCurrent(ASXCode)
