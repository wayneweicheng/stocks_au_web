...
AS
/******************************************************************************
File: usp_GetRecentPrimeBuyCloseSellOpenStock.sql
Stored Procedure Name: usp_GetRecentPrimeBuyCloseSellOpenStock
Overview
-----------------
usp_GetRecentPrimeBuyCloseSellOpenStock
...
BEGIN --Proc
	...
	BEGIN TRY
		...
		DECLARE @vchProcedureName AS VARCHAR(100);		SET @vchProcedureName = 'usp_GetRecentPrimeBuyCloseSellOpenStock'
		...
		from
		(
			select a.ASXCode, x.IntraDayChange, x.[Close], x.[Open], x.ObservationDate, x.PrevObservationDate, (x.[Open]-a.[Close])*100.0/x.[Open] as OpenVsPrevClose, a.[Close]*a.Volume as [Value]
			from #TempPriceHistory as a
			left join
			(
				select ASXCode, ObservationDate, PrevObservationDate, ([Close] - [Open])*100.0/[Open] as IntraDayChange, [Close], [Open]
