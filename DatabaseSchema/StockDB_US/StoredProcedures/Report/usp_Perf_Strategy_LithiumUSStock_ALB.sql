-- Stored procedure: [Report].[usp_Perf_Strategy_LithiumUSStock_ALB]


CREATE PROCEDURE [Report].[usp_Perf_Strategy_LithiumUSStock_ALB]
@pbitDebug AS BIT = 0,
@pintErrorNumber AS INT = 0 OUTPUT,
@pvchASXCode as varchar(10)
AS
/******************************************************************************
File: usp_Perf_Strategy_LithiumUSStock_ALB.sql
Stored Procedure Name: usp_Perf_Strategy_LithiumUSStock_ALB
Overview
-----------------
usp_Perf_Strategy_LithiumUSStock_ALB
...
BEGIN --Proc
	...
	BEGIN TRY
		...
		DECLARE @vchProcedureName AS VARCHAR(100);		SET @vchProcedureName = 'usp_Perf_Strategy_LithiumUSStock_ALB'
		...
		select 
			...
			case when SwingIndicator = 'Swing Up' then a.TodayOpenToCloseChange
					when SwingIndicator = 'Swing Down' then -1*b.TodayOpenToCloseChange
			end as TomorrowOpenToCloseChange, 
			null as TomorrowChange,
			c.SwingIndicator,
			a.ObservationDate,
		...
		inner join
		(
			select ObservationDate, TodayChange,
				case when TodayChange > 5 then 'Swing Up'
					 when TodayChange < -5 then 'Swing Down'
				end as SwingIndicator
			from StockDB.StockData.v_PriceHistory
			where ASXCode = 'PLS.AX'
			and (TodayChange > 5 or TodayChange < -5)
			and ObservationDate > '2021-01-01'
		) as c
		on a.ObservationDate = c.ObservationDate
		where a.ASXCode = @pvchASXCode
		and b.ASXCode = @pvchASXCode
