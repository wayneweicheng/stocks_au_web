-- Stored procedure: [Report].[usp_GetWatchList]
...
CREATE PROCEDURE [Report].[usp_GetWatchList]
@pbitDebug AS BIT = 0,
...
BEGIN --Proc
	...
	BEGIN TRY
		...
		--Code goes here 
		--begin transaction
		if object_id(N'Tempdb.dbo.#TempWL') is not null
			drop table #TempWL

		select x.WatchListName, stuff((
			select ',' + [ASXCode]
			from StockData.WatchListStock as a
			where x.WatchListName = a.WatchListName
			order by ASXCode asc
			for xml path('')), 1, 1, ''
		) as [ASXCode]
		into #TempWL
		from StockData.WatchListStock as x
		where 1 = 1
		group by x.WatchListName
